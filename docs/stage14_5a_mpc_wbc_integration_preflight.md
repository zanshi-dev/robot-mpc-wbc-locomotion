# Stage 14.5A MPC-to-WBC / MuJoCo Integration Preflight

Scope: simulation-only inventory preflight.

This stage does not run MuJoCo closed-loop simulation, does not run WBC/QP, does not generate joint torque candidates, and does not modify the frozen mixed baseline control law.

## Generated evidence

- Candidate file list: `results/logs_sample/stage14_5a_candidate_files.txt`
- Summary JSON: `results/logs_sample/stage14_5a_mpc_wbc_integration_preflight_summary.json`

## Safety flags

- simulation_only_project: True
- hardware_deployment_completed: False
- torque_enable_ready: False
- torque_publisher_enabled: False
- control_law_changed: False
- mixed_baseline_modified: False
- mujoco_torque_used: False
- ros_publisher_used: False

## Inventory summary

- pass: True
- failed_checks: []
- inspected_file_count: 626
- candidate_file_count: 424

## Category matches

### stage14_4_mpc

count: 19

- `README.md`
- `docs/CONTROL_ARCHITECTURE_OVERVIEW.md`
- `docs/ONE_PAGE_TECHNICAL_REPORT.md`
- `docs/stage14_4_base_velocity_tracking_mpc.md`
- `docs/stage14_4b_base_velocity_tracking_mpc_validation.md`
- `docs/stage14_4c_mpc_scope_explanation.md`
- `docs/stage14_4d_document_language_audit.md`
- `results/logs_sample/stage14_4_base_velocity_tracking_mpc_summary.json`
- `results/logs_sample/stage14_4b_base_velocity_tracking_mpc_validation_summary.json`
- `results/logs_sample/stage14_4c_mpc_scope_explanation_summary.json`
- `results/logs_sample/stage14_4d_document_language_audit_summary.json`
- `results/logs_sample/stage14_4e_entry_docs_update_summary.json`
- `results/logs_sample/stage14_4e_entry_docs_validation_summary.json`
- `scripts/stage14_4_base_velocity_tracking_mpc_demo.py`
- `scripts/stage14_4b_validate_base_velocity_mpc_rollout.py`
- `scripts/stage14_4c_validate_mpc_scope_explanation.py`
- `scripts/stage14_4d_audit_document_language.py`
- `scripts/stage14_4e_update_entry_docs.py`
- `scripts/stage14_4e_validate_entry_docs.py`

### mujoco_or_baseline

count: 267

- `PROJECT_STATUS.md`
- `README.md`
- `assets/go1/README.md`
- `assets/go1/go1.xml`
- `assets/go1/scene.xml`
- `assets/stage00_minimal_leg.xml`
- `docs/CONTROL_ARCHITECTURE_OVERVIEW.md`
- `docs/CPP_CONTROL_ALGORITHMS.md`
- `docs/DEMO_VIDEO_MANIFEST.md`
- `docs/FINAL_PACKAGE_WITH_DEMO_VIDEO_MANIFEST.md`
- `docs/ONE_PAGE_TECHNICAL_REPORT.md`
- `docs/REPORT_READY_CLAIMS_AND_LIMITATIONS.md`
- `docs/SIMULATION_ONLY_RESULTS_SUMMARY.md`
- `docs/SIMULATION_ONLY_SCOPE.md`
- `docs/STAGE06_QP_FORCE_TO_ACTUATOR_TORQUE.md`
- `docs/STAGE06_QP_TORQUE_SUPPORT_TEST.md`
- `docs/STAGE06_SUMMARY_AND_STAGE07_INTERFACE.md`
- `docs/STAGE07_CONSOLIDATED_SUMMARY_AFTER_SWING.md`
- `docs/STAGE07_CONTACT_MODE_SEQUENCE_RAMP_TEST.md`
- `docs/STAGE07_CONTACT_MODE_TORQUE_RAMP_CHECK.md`
- `docs/STAGE07_CONTACT_SCHEDULE_WBC_QP.md`
- `docs/STAGE07_FINAL_SUMMARY.md`
- `docs/STAGE07_FINAL_UPDATE_AFTER_MODE_SEQUENCE.md`
- `docs/STAGE07_FULL_WBC_BASE_VERTICAL_ACCEL_TASK_QP.md`
- `docs/STAGE07_FULL_WBC_DYNAMICS_QP.md`
- `docs/STAGE07_FULL_WBC_FINAL_UPDATE.md`
- `docs/STAGE07_FULL_WBC_STANCE_CONSTRAINT_QP.md`
- `docs/STAGE07_FULL_WBC_SUMMARY.md`
- `docs/STAGE07_FULL_WBC_SWING_ACCEL_TASK_QP.md`
- `docs/STAGE07_FULL_WBC_TORQUE_RECONSTRUCTION_AND_RAMP_CHECK.md`
- `docs/STAGE07_FULL_WBC_TORQUE_SEQUENCE_SUPPORT_TEST.md`
- `docs/STAGE07_MINIMAL_WBC_TORQUE_QP.md`
- `docs/STAGE07_ONLINE_FULL_WBC_STEP_LOOP_PROTO.md`
- `docs/STAGE07_ONLINE_FULL_WBC_WITH_SCHEDULER_PROTO.md`
- `docs/STAGE07_ONLINE_LOCOMOTION_CONSOLIDATED_SUMMARY.md`
- `docs/STAGE07_ONLINE_STANCE_PD_WBC_PLUS_SWING_PD_RECOMMENDED_TEST.md`
- `docs/STAGE07_ONLINE_SWING_TRAJECTORY_TRACKING_CHECK.md`
- `docs/STAGE07_SUMMARY_AND_NEXT_STEPS.md`
- `docs/STAGE07_SWING_FOOT_TRACKING_QP.md`
- `docs/STAGE07_SWING_JOINT_TARGET_SEQUENCE.md`
- `docs/STAGE07_SWING_JOINT_TARGET_TRACKING_RECOMMENDED_TEST.md`
- `docs/STAGE07_SWING_TRAJECTORY_QP.md`
- `docs/STAGE07_WBC_BASE_WRENCH_QP.md`
- `docs/STAGE07_WBC_BASE_WRENCH_SUPPORT_TEST.md`
- `docs/STAGE07_WBC_TORQUE_SUPPORT_TEST.md`
- `docs/STAGE08_0_7_RUNTIME_INTERFACE_HARDENING_SUMMARY.md`
- `docs/STAGE08_ACTIVE_LEG_ORDER_REFACTOR_AND_REGRESSION.md`
- `docs/STAGE08_ADAPTER_BACKED_STAGE07_BASELINE_AB_TEST.md`
- `docs/STAGE08_ADAPTER_ZERO_CONTROL_REGRESSION_GUARD.md`
- `docs/STAGE08_BASELINE_FREEZE_MANIFEST.md`
- `docs/STAGE08_FREEZE_INTEGRITY_CHECK.md`
- `docs/STAGE08_RUNTIME_INTERFACE_ADAPTER_MODULE_CHECK.md`
- `docs/STAGE08_RUNTIME_INTERFACE_CONTRACT_CHECK.md`
- `docs/STAGE08_RUNTIME_MAPPING_AUDIT_TRIAGE.md`
- `docs/STAGE08_RUNTIME_MAPPING_DUPLICATION_AUDIT.md`
- `docs/STAGE09_0_6_INTERFACE_MIRROR_FREEZE_SUMMARY.md`
- `docs/STAGE09_CPP_MIRROR_CONTRACT_REPORT.md`
- `docs/STAGE09_PYTHON_BASELINE_ROS2_FIELD_MAPPING.md`
- `docs/STAGE09_ROS2_CPP_INTERFACE_CONTRACT_INVENTORY.md`
- `docs/STAGE09_ROS2_RUNTIME_MIRROR_SMOKE_TEST.md`
- `docs/STAGE10_0_11_FULL_NO_PUBLISHER_CONTROLLER_FREEZE.md`
- `docs/STAGE10_0_5_CONTROLLER_PLANNING_FREEZE_SUMMARY.md`
- `docs/STAGE10_7_8_SAFETY_UTILITY_FREEZE_SUMMARY.md`
- `docs/STAGE10_CONTROLLER_IMPLEMENTATION_PLAN_AND_SAFETY_GATE.md`
- `docs/STAGE10_PYTHON_FROZEN_BASELINE_AB_REGRESSION.md`
- `docs/STAGE10_TORQUE_PUBLISHER_ENABLE_GATE_DESIGN.md`
- `docs/STAGE11_0_2_PUBLISHER_PATH_PLANNING_FREEZE_SUMMARY.md`
- `docs/STAGE11_0_8_FULL_PUBLISHER_PATH_NO_ACTIVE_PUBLISHER_FREEZE.md`
- `docs/STAGE11_5_7_DORMANT_PUBLISHER_RUNTIME_FREEZE_SUMMARY.md`
- `docs/STAGE11_DORMANT_PUBLISHER_PATH_SOURCE_SKELETON_FREEZE_SUMMARY.md`
- `docs/STAGE11_FULL_FREEZE_INTEGRITY_CHECK.md`
- `docs/STAGE12_CONSTRUCTION_STAGE_PREFLIGHT_FREEZE.md`
- `docs/WBC_QP_EXPLAINED.md`
- `docs/interview/INTERVIEW_3MIN_SYSTEM_EXPLANATION.md`
- `docs/stage01_ros2_mujoco_bridge.md`
- `docs/stage02_pinocchio_kinematics_validation.md`
- `docs/stage03_standing_pd_baseline.md`
- `docs/stage04_gait_swing_foothold.md`
- `docs/stage05_standing_contact_force_qp.md`
- `docs/stage06_force_to_joint_torque.md`
- `docs/stage12_21_r3_final_repaired_freeze_summary.md`
- `docs/stage12_22_simulation_only_scope_freeze.md`
- `docs/stage13_0_simulation_only_locomotion_preflight_inventory.md`
- `docs/stage13_1a_existing_stage7_baseline_summary_verification.md`
- `docs/stage13_1b_rerun_1200step_simulation_only_mixed_baseline.md`
- `docs/stage13_2b_run_2400step_simulation_only_robustness_regression.md`
- `docs/stage13_3_report_ready_results_packaging.md`
- `docs/stage13_5a_mujoco_offscreen_rollout_demo_video.md`
- `docs/stage13_5a_r1_mujoco_offscreen_rollout_video_640fb.md`
- `docs/stage13_5a_r2_mujoco_offscreen_rollout_video_fixed_ffmpeg.md`
- `docs/stage13_5b_demo_video_evidence_freeze.md`
- `docs/stage14_4_base_velocity_tracking_mpc.md`
- `docs/stage14_4b_base_velocity_tracking_mpc_validation.md`
- `docs/stage14_4c_mpc_scope_explanation.md`
- `docs/stage14_4d_document_language_audit.md`
- `requirements.txt`
- `results/logs_sample/stage00_env.txt`
- `results/logs_sample/stage08_6_backup_stage07_online_full_wbc_scheduler_recommended_run.py.txt`
- `results/logs_sample/stage08_6_backup_stage07_online_stance_pd_wbc_plus_swing_pd_recommended_test.py.txt`
- `results/logs_sample/stage09_cpp_mirror_runtime_contract_guard_bridge_stderr.txt`
- `results/logs_sample/stage09_ros2_runtime_mirror_bridge_stderr.txt`
- `results/logs_sample/stage10_clamp_watchdog_bridge_stderr.txt`
- `results/logs_sample/stage10_cpp_state_cache_runtime_validation_bridge_stderr.txt`
- `results/logs_sample/stage10_disabled_controller_uses_safety_bridge_stderr.txt`
- `results/logs_sample/stage10_manual_enable_params_bridge_stderr.txt`
- `results/logs_sample/stage10_zero_torque_dry_run_bridge_stderr.txt`
- `results/logs_sample/stage11_dormant_publisher_path_source_skeleton_bridge_stderr.txt`
- `results/logs_sample/stage11_publisher_path_source_guard_bridge_stderr.txt`
- `results/logs_sample/stage11_runtime_guard_hardening_bridge_stderr.txt`
- `results/logs_sample/stage12_22_simulation_only_scope_freeze_summary.json`
- `results/logs_sample/stage12_bounded_one_shot_publish_call_freeze_bridge_stderr.txt`
- `results/logs_sample/stage12_bounded_one_shot_publish_call_repair2_bridge_stderr.txt`
- `results/logs_sample/stage12_bounded_one_shot_publish_call_repair_bridge_stderr.txt`
- `results/logs_sample/stage12_bounded_one_shot_zero_safe_publish_call_bridge_stderr.txt`
- `results/logs_sample/stage12_manual_enable_runtime_activation_bridge_stderr.txt`
- `results/logs_sample/stage12_pre_construction_guard_bridge_stderr.txt`
- `results/logs_sample/stage12_publisher_construction_without_publish_bridge_stderr.txt`
- `results/logs_sample/stage13_0_simulation_only_locomotion_preflight_inventory_summary.json`
- `results/logs_sample/stage13_1a_existing_stage7_baseline_summary_verification_summary.json`
- `results/logs_sample/stage13_2a_r1_runner_structure_inspection_summary.json`
- `results/logs_sample/stage13_2b_run_2400step_simulation_only_robustness_regression_summary.json`
- `results/logs_sample/stage13_3_report_ready_results_packaging_summary.json`
- `results/logs_sample/stage13_5a_mujoco_offscreen_rollout_demo_video_summary.json`
- `results/logs_sample/stage13_5a_r1_mujoco_offscreen_rollout_video_640fb_summary.json`
- `results/logs_sample/stage13_5a_r2_mujoco_offscreen_rollout_video_fixed_ffmpeg_summary.json`
- `results/logs_sample/stage13_5b_demo_video_evidence_freeze_summary.json`
- `results/logs_sample/stage13_5c_final_package_with_demo_video_manifest.json`
- `results/logs_sample/stage13_5c_final_package_with_demo_video_manifest_summary.json`
- `results/logs_sample/stage14_2_readme_interview_entry_summary.json`
- `results/logs_sample/stage14_4_base_velocity_tracking_mpc_summary.json`
- `results/logs_sample/stage14_4b_base_velocity_tracking_mpc_validation_summary.json`
- `results/logs_sample/stage14_4c_mpc_scope_explanation_summary.json`
- `results/logs_sample/stage14_4d_document_language_audit_summary.json`
- `results/logs_sample/stage14_4e_entry_docs_update_summary.json`
- `results/logs_sample/stage14_4e_entry_docs_validation_summary.json`
- `ros2_ws/src/robot_mpc_wbc_bridge/launch/mujoco_bridge.launch.py`
- `ros2_ws/src/robot_mpc_wbc_bridge/robot_mpc_wbc_bridge/mujoco_bridge_node.py`
- `ros2_ws/src/robot_mpc_wbc_bridge/setup.py`
- `scripts/common/go1_runtime_interface.py`
- `scripts/stage00_inspect_go1_model.py`
- `scripts/stage00_torque_control_demo.py`
- `scripts/stage02_check_pinocchio_jacobian.py`
- `scripts/stage02_compare_foot_velocity.py`
- `scripts/stage02_compare_full_foot_velocity.py`
- `scripts/stage02_compare_joint_order.py`
- `scripts/stage02_compare_mujoco_pinocchio_fk.py`
- `scripts/stage02_compare_mujoco_pinocchio_jacobian.py`
- `scripts/stage02_validate_pinocchio_fk.py`
- `scripts/stage03_standing_pd_demo.py`
- `scripts/stage04_open_loop_trot_pd_demo.py`
- `scripts/stage05_centroidal_z_mpc.py`
- `scripts/stage05_contact_schedule_force_qp.py`
- `scripts/stage05_horizon_force_qp.py`
- `scripts/stage05_standing_contact_force_qp.py`
- `scripts/stage06_force_sign_sanity_check.py`
- `scripts/stage06_jacobian_transpose_torque.py`
- `scripts/stage06_qp_force_to_torque.py`
- `scripts/stage06_qp_torque_support_test.py`
- `scripts/stage07_contact_mode_sequence_ramp_test.py`
- `scripts/stage07_contact_schedule_wbc_qp.py`
- `scripts/stage07_contact_schedule_wbc_scale_sweep.py`
- `scripts/stage07_contact_schedule_wbc_scaled_support_test.py`
- `scripts/stage07_contact_schedule_wbc_support_test.py`
- `scripts/stage07_full_wbc_base_accel_task_qp.py`
- `scripts/stage07_full_wbc_base_vertical_accel_task_qp.py`
- `scripts/stage07_full_wbc_dynamics_qp.py`
- `scripts/stage07_full_wbc_stance_constraint_qp.py`
- `scripts/stage07_full_wbc_swing_accel_task_qp.py`
- `scripts/stage07_full_wbc_torque_reconstruction_check.py`
- `scripts/stage07_full_wbc_torque_sequence_support_test.py`
- `scripts/stage07_online_full_wbc_plus_swing_joint_tracking_proto.py`
- `scripts/stage07_online_full_wbc_plus_swing_joint_tracking_sweep.py`
- `scripts/stage07_online_full_wbc_scheduler_recommended_run.py`
- `scripts/stage07_online_full_wbc_scheduler_stability_sweep.py`
- `scripts/stage07_online_full_wbc_step_loop_proto.py`
- `scripts/stage07_online_full_wbc_with_scheduler_proto.py`
- `scripts/stage07_online_stance_pd_wbc_plus_swing_pd_recommended_test.py`
- `scripts/stage07_online_stance_pd_wbc_plus_swing_pd_sweep.py`
- `scripts/stage07_online_stance_wbc_plus_swing_pd_sweep.py`
- `scripts/stage07_online_swing_joint_target_tracking_support_test.py`
- `scripts/stage07_online_swing_joint_tracking_recommended_test.py`
- `scripts/stage07_online_swing_joint_tracking_stability_sweep.py`
- `scripts/stage07_online_swing_trajectory_memory_proto.py`
- `scripts/stage07_online_swing_trajectory_proto.py`
- `scripts/stage07_online_swing_trajectory_tracking_check.py`
- `scripts/stage07_swing_foot_tracking_qp.py`
- `scripts/stage07_swing_joint_target_tracking_recommended_test.py`
- `scripts/stage07_swing_joint_target_tracking_recommended_test_flrr.py`
- `scripts/stage07_swing_joint_target_tracking_test.py`
- `scripts/stage07_swing_joint_target_tracking_with_wbc_test.py`
- `scripts/stage07_swing_tracking_mode_sequence_test.py`
- `scripts/stage07_swing_tracking_stability_sweep.py`
- `scripts/stage07_swing_trajectory_qp.py`
- `scripts/stage07_swing_trajectory_qp_k9.py`
- `scripts/stage07_wbc_base_wrench_qp.py`
- `scripts/stage07_wbc_base_wrench_support_test.py`
- `scripts/stage07_wbc_posture_regularized_qp.py`
- `scripts/stage07_wbc_posture_regularized_support_test.py`
- `scripts/stage07_wbc_torque_support_test.py`
- `scripts/stage08_active_leg_order_refactor_and_regression.py`
- `scripts/stage08_adapter_backed_stage07_baseline_ab_test.py`
- `scripts/stage08_adapter_backed_stage07_recommended_test.py`
- `scripts/stage08_adapter_zero_control_regression_guard.py`
- `scripts/stage08_freeze_integrity_check.py`
- `scripts/stage08_runtime_interface_adapter_module_check.py`
- `scripts/stage08_runtime_interface_contract_check.py`
- `scripts/stage08_runtime_mapping_duplication_audit.py`
- `scripts/stage09_cpp_mirror_contract_report.py`
- `scripts/stage09_cpp_mirror_runtime_contract_guard.py`
- `scripts/stage09_interface_mirror_freeze_summary.py`
- `scripts/stage09_python_baseline_ros2_field_mapping.py`
- `scripts/stage09_ros2_cpp_interface_contract_inventory.py`
- `scripts/stage09_ros2_runtime_mirror_smoke_test.py`
- `scripts/stage10_0_11_full_no_publisher_controller_freeze.py`
- `scripts/stage10_0_5_controller_planning_freeze_summary.py`
- `scripts/stage10_7_8_safety_utility_freeze_summary.py`
- `scripts/stage10_clamp_watchdog_utility_without_publisher.py`
- `scripts/stage10_controller_implementation_plan_and_safety_gate.py`
- `scripts/stage10_cpp_state_cache_runtime_validation.py`
- `scripts/stage10_disabled_controller_uses_safety_utilities.py`
- `scripts/stage10_manual_enable_params_disabled_without_publisher.py`
- `scripts/stage10_python_frozen_baseline_ab_regression.py`
- `scripts/stage10_torque_publisher_enable_gate_design.py`
- `scripts/stage10_zero_torque_dry_run_internal_validation.py`
- `scripts/stage11_0_2_publisher_path_planning_freeze_summary.py`
- `scripts/stage11_0_8_full_publisher_path_no_active_publisher_freeze.py`
- `scripts/stage11_5_7_dormant_publisher_runtime_freeze_summary.py`
- `scripts/stage11_disabled_publisher_path_skeleton_design_only.py`
- `scripts/stage11_disabled_publisher_path_skeleton_preflight.py`
- `scripts/stage11_dormant_publisher_path_source_skeleton.py`
- `scripts/stage11_dormant_publisher_path_source_skeleton_freeze_summary.py`
- `scripts/stage11_full_freeze_integrity_check.py`
- `scripts/stage11_publisher_path_skeleton_planning_only.py`
- `scripts/stage11_publisher_path_source_guard_before_implementation.py`
- `scripts/stage11_runtime_guard_hardening_for_dormant_publisher_skeleton.py`
- `scripts/stage12_21_r3_final_repaired_freeze_summary.py`
- `scripts/stage12_22_simulation_only_scope_freeze.py`
- `scripts/stage12_active_publisher_construction_planning_only.py`
- `scripts/stage12_bounded_one_shot_publish_call_freeze_regression.py`
- `scripts/stage12_bounded_one_shot_publish_call_repair.py`
- `scripts/stage12_bounded_one_shot_publish_call_repair2.py`
- `scripts/stage12_bounded_one_shot_zero_safe_publish_call_source_patch.py`
- `scripts/stage12_construction_stage_preflight_freeze.py`
- `scripts/stage12_manual_enable_runtime_activation_without_publish.py`
- `scripts/stage12_pre_construction_source_runtime_guard.py`
- `scripts/stage12_publisher_construction_source_patch_without_publish.py`
- `scripts/stage13_0_simulation_only_locomotion_preflight_inventory.py`
- `scripts/stage13_1a_existing_stage7_baseline_summary_verification.py`
- `scripts/stage13_1b_rerun_1200step_simulation_only_mixed_baseline.py`
- `scripts/stage13_2_2400step_online_full_wbc_scheduler_runner.py`
- `scripts/stage13_2_2400step_simulation_only_mixed_baseline_runner.py`
- `scripts/stage13_2a_2400step_preflight_and_derived_runner.py`
- `scripts/stage13_2a_r2_create_2400step_derived_runner.py`
- `scripts/stage13_2b_run_2400step_simulation_only_robustness_regression.py`
- `scripts/stage13_2c_final_2400step_robustness_evidence_freeze.py`
- `scripts/stage13_3_report_ready_results_packaging.py`
- `scripts/stage13_5a_mujoco_offscreen_rollout_demo_video.py`
- `scripts/stage13_5a_r1_mujoco_offscreen_rollout_video_640fb.py`
- `scripts/stage13_5a_r2_mujoco_offscreen_rollout_video_fixed_ffmpeg.py`
- `scripts/stage13_5b_demo_video_evidence_freeze.py`
- `scripts/stage13_5c_final_package_with_demo_video_manifest.py`
- `scripts/stage14_4_base_velocity_tracking_mpc_demo.py`
- `scripts/stage14_4b_validate_base_velocity_mpc_rollout.py`
- `scripts/stage14_4c_validate_mpc_scope_explanation.py`
- `scripts/stage14_4d_audit_document_language.py`
- `scripts/stage14_4e_update_entry_docs.py`
- `scripts/stage14_4e_validate_entry_docs.py`

### wbc_or_qp

count: 331

- `PROJECT_STATUS.md`
- `README.md`
- `docs/CONTROL_ARCHITECTURE_OVERVIEW.md`
- `docs/CPP_CONTROL_ALGORITHMS.md`
- `docs/DEMO_VIDEO_MANIFEST.md`
- `docs/FINAL_PACKAGE_WITH_DEMO_VIDEO_MANIFEST.md`
- `docs/ONE_PAGE_TECHNICAL_REPORT.md`
- `docs/REPORT_READY_PACKAGE_MANIFEST.md`
- `docs/REPORT_READY_RESULTS.md`
- `docs/SIMULATION_ONLY_RESULTS_SUMMARY.md`
- `docs/STAGE06_QP_FORCE_TO_ACTUATOR_TORQUE.md`
- `docs/STAGE06_QP_TORQUE_SUPPORT_TEST.md`
- `docs/STAGE06_SUMMARY_AND_STAGE07_INTERFACE.md`
- `docs/STAGE07_CONSOLIDATED_SUMMARY_AFTER_SWING.md`
- `docs/STAGE07_CONTACT_MODE_SEQUENCE_RAMP_TEST.md`
- `docs/STAGE07_CONTACT_MODE_TRANSITION_CHECK.md`
- `docs/STAGE07_CONTACT_SCHEDULE_WBC_QP.md`
- `docs/STAGE07_CONTACT_SCHEDULE_WBC_SCALED_SUPPORT_TEST.md`
- `docs/STAGE07_FINAL_SUMMARY.md`
- `docs/STAGE07_FINAL_UPDATE_AFTER_MODE_SEQUENCE.md`
- `docs/STAGE07_FULL_WBC_BASE_VERTICAL_ACCEL_TASK_QP.md`
- `docs/STAGE07_FULL_WBC_DYNAMICS_QP.md`
- `docs/STAGE07_FULL_WBC_FINAL_UPDATE.md`
- `docs/STAGE07_FULL_WBC_STANCE_CONSTRAINT_QP.md`
- `docs/STAGE07_FULL_WBC_SUMMARY.md`
- `docs/STAGE07_FULL_WBC_SWING_ACCEL_TASK_QP.md`
- `docs/STAGE07_FULL_WBC_TORQUE_RECONSTRUCTION_AND_RAMP_CHECK.md`
- `docs/STAGE07_FULL_WBC_TORQUE_SEQUENCE_SUPPORT_TEST.md`
- `docs/STAGE07_GAIT_PHASE_SCHEDULER_PROTO.md`
- `docs/STAGE07_MINIMAL_WBC_TORQUE_QP.md`
- `docs/STAGE07_ONLINE_FULL_WBC_PLUS_SWING_JOINT_TRACKING_SWEEP.md`
- `docs/STAGE07_ONLINE_FULL_WBC_SCHEDULER_RECOMMENDED_RUN.md`
- `docs/STAGE07_ONLINE_FULL_WBC_SCHEDULER_STABILITY_SWEEP.md`
- `docs/STAGE07_ONLINE_FULL_WBC_STEP_LOOP_PROTO.md`
- `docs/STAGE07_ONLINE_FULL_WBC_WITH_SCHEDULER_PROTO.md`
- `docs/STAGE07_ONLINE_LOCOMOTION_CONSOLIDATED_SUMMARY.md`
- `docs/STAGE07_ONLINE_STANCE_PD_WBC_PLUS_SWING_PD_RECOMMENDED_TEST.md`
- `docs/STAGE07_ONLINE_STANCE_PD_WBC_PLUS_SWING_PD_SWEEP.md`
- `docs/STAGE07_ONLINE_SWING_JOINT_TRACKING_RECOMMENDED_TEST.md`
- `docs/STAGE07_ONLINE_SWING_JOINT_TRACKING_STABILITY_SWEEP.md`
- `docs/STAGE07_ONLINE_SWING_TRAJECTORY_MEMORY_PROTO.md`
- `docs/STAGE07_ONLINE_SWING_TRAJECTORY_TRACKING_CHECK.md`
- `docs/STAGE07_SUMMARY_AND_NEXT_STEPS.md`
- `docs/STAGE07_SWING_FOOT_TRACKING_QP.md`
- `docs/STAGE07_SWING_JOINT_TARGET_SEQUENCE.md`
- `docs/STAGE07_SWING_JOINT_TARGET_TRACKING_RECOMMENDED_TEST.md`
- `docs/STAGE07_SWING_TRACKING_MODE_SEQUENCE_TEST.md`
- `docs/STAGE07_SWING_TRACKING_RECOMMENDED_BOTH_MODES.md`
- `docs/STAGE07_SWING_TRAJECTORY_QP.md`
- `docs/STAGE07_WBC_BASE_WRENCH_QP.md`
- `docs/STAGE07_WBC_BASE_WRENCH_SUPPORT_TEST.md`
- `docs/STAGE07_WBC_TORQUE_SUPPORT_TEST.md`
- `docs/STAGE07_WBC_VARIANT_COMPARISON.md`
- `docs/STAGE08_0_7_RUNTIME_INTERFACE_HARDENING_SUMMARY.md`
- `docs/STAGE08_ACTIVE_LEG_ORDER_REFACTOR_AND_REGRESSION.md`
- `docs/STAGE08_ADAPTER_BACKED_STAGE07_BASELINE_AB_TEST.md`
- `docs/STAGE08_ADAPTER_ZERO_CONTROL_REGRESSION_GUARD.md`
- `docs/STAGE08_BASELINE_FREEZE_MANIFEST.md`
- `docs/STAGE08_FREEZE_INTEGRITY_CHECK.md`
- `docs/STAGE08_RECOMMENDED_RUNTIME_SAFE_ENTRYPOINT.md`
- `docs/STAGE08_RUNTIME_INTERFACE_ADAPTER_MODULE_CHECK.md`
- `docs/STAGE08_RUNTIME_INTERFACE_CONTRACT_CHECK.md`
- `docs/STAGE08_RUNTIME_MAPPING_AUDIT_TRIAGE.md`
- `docs/STAGE08_RUNTIME_MAPPING_DUPLICATION_AUDIT.md`
- `docs/STAGE09_0_6_INTERFACE_MIRROR_FREEZE_SUMMARY.md`
- `docs/STAGE09_CPP_MIRROR_CONTRACT_REPORT.md`
- `docs/STAGE09_CPP_MIRROR_RUNTIME_CONTRACT_GUARD.md`
- `docs/STAGE09_PYTHON_BASELINE_ROS2_FIELD_MAPPING.md`
- `docs/STAGE09_ROS2_CPP_INTERFACE_CONTRACT_INVENTORY.md`
- `docs/STAGE09_ROS2_CPP_INTERFACE_MIRROR_SKELETON.md`
- `docs/STAGE09_ROS2_RUNTIME_MIRROR_SMOKE_TEST.md`
- `docs/STAGE09_ROS2_TOPIC_SCHEMA_SNAPSHOT.md`
- `docs/STAGE10_0_11_FULL_NO_PUBLISHER_CONTROLLER_FREEZE.md`
- `docs/STAGE10_0_5_CONTROLLER_PLANNING_FREEZE_SUMMARY.md`
- `docs/STAGE10_7_8_SAFETY_UTILITY_FREEZE_SUMMARY.md`
- `docs/STAGE10_CLAMP_WATCHDOG_UTILITY_WITHOUT_PUBLISHER.md`
- `docs/STAGE10_CONTROLLER_IMPLEMENTATION_PLAN_AND_SAFETY_GATE.md`
- `docs/STAGE10_CPP_STATE_CACHE_RUNTIME_VALIDATION.md`
- `docs/STAGE10_DISABLED_CONTROLLER_USES_SAFETY_UTILITIES.md`
- `docs/STAGE10_DISABLED_CPP_CONTROLLER_SKELETON.md`
- `docs/STAGE10_MANUAL_ENABLE_PARAMS_DISABLED_WITHOUT_PUBLISHER.md`
- `docs/STAGE10_MANUAL_ENABLE_PARAM_GUARD_FREEZE_SUMMARY.md`
- `docs/STAGE10_PYTHON_FROZEN_BASELINE_AB_REGRESSION.md`
- `docs/STAGE10_TORQUE_PUBLISHER_ENABLE_GATE_DESIGN.md`
- `docs/STAGE10_ZERO_TORQUE_DRY_RUN_INTERNAL_VALIDATION.md`
- `docs/STAGE11_0_2_PUBLISHER_PATH_PLANNING_FREEZE_SUMMARY.md`
- `docs/STAGE11_0_8_FULL_PUBLISHER_PATH_NO_ACTIVE_PUBLISHER_FREEZE.md`
- `docs/STAGE11_5_7_DORMANT_PUBLISHER_RUNTIME_FREEZE_SUMMARY.md`
- `docs/STAGE11_DISABLED_PUBLISHER_PATH_SKELETON_DESIGN_ONLY.md`
- `docs/STAGE11_DISABLED_PUBLISHER_PATH_SKELETON_PREFLIGHT.md`
- `docs/STAGE11_DORMANT_PUBLISHER_PATH_SOURCE_SKELETON.md`
- `docs/STAGE11_DORMANT_PUBLISHER_PATH_SOURCE_SKELETON_FREEZE_SUMMARY.md`
- `docs/STAGE11_FULL_FREEZE_INTEGRITY_CHECK.md`
- `docs/STAGE11_PUBLISHER_PATH_SKELETON_PLANNING_ONLY.md`
- `docs/STAGE11_PUBLISHER_PATH_SOURCE_GUARD_BEFORE_IMPLEMENTATION.md`
- `docs/STAGE11_RUNTIME_GUARD_HARDENING_FOR_DORMANT_PUBLISHER_SKELETON.md`
- `docs/STAGE12_ACTIVE_PUBLISHER_CONSTRUCTION_PLANNING_ONLY.md`
- `docs/STAGE12_BOUNDED_ONE_SHOT_PUBLISH_CALL_FREEZE_REGRESSION.md`
- `docs/STAGE12_BOUNDED_ONE_SHOT_PUBLISH_CALL_REPAIR.md`
- `docs/STAGE12_BOUNDED_ONE_SHOT_ZERO_SAFE_PUBLISH_CALL_SOURCE_PATCH.md`
- `docs/STAGE12_BOUNDED_PUBLISH_CALL_SOURCE_PATCH_DESIGN_ONLY.md`
- `docs/STAGE12_BOUNDED_PUBLISH_CALL_SOURCE_PATCH_PREFLIGHT_FREEZE.md`
- `docs/STAGE12_BOUNDED_ZERO_SAFE_PUBLISH_CALL_IMPLEMENTATION_PLAN.md`
- `docs/STAGE12_CONSTRUCTION_STAGE_PREFLIGHT_FREEZE.md`
- `docs/STAGE12_CONTINUOUS_TORQUE_STREAMING_DESIGN_ONLY.md`
- `docs/STAGE12_CONTINUOUS_TORQUE_STREAMING_PREFLIGHT_FREEZE.md`
- `docs/STAGE12_CONTINUOUS_TORQUE_STREAMING_SOURCE_PATCH_DESIGN_ONLY.md`
- `docs/STAGE12_CONTINUOUS_TORQUE_STREAMING_SOURCE_PATCH_PREFLIGHT_FREEZE.md`
- `docs/STAGE12_MANUAL_ENABLE_ACTIVATION_DESIGN_ONLY.md`
- `docs/STAGE12_MANUAL_ENABLE_NO_PUBLISH_FREEZE.md`
- `docs/STAGE12_MANUAL_ENABLE_RUNTIME_ACTIVATION_WITHOUT_PUBLISH.md`
- `docs/STAGE12_PRE_CONSTRUCTION_SOURCE_RUNTIME_GUARD.md`
- `docs/STAGE12_PUBLISHER_CONSTRUCTION_NO_PUBLISH_FREEZE.md`
- `docs/STAGE12_PUBLISHER_CONSTRUCTION_SOURCE_PATCH_DESIGN_ONLY.md`
- `docs/STAGE12_PUBLISHER_CONSTRUCTION_SOURCE_PATCH_WITHOUT_PUBLISH.md`
- `docs/STAGE12_PUBLISH_CALL_DESIGN_ONLY.md`
- `docs/STAGE12_PUBLISH_CALL_PREFLIGHT_FREEZE.md`
- `docs/WBC_QP_EXPLAINED.md`
- `docs/interview/INTERVIEW_3MIN_SYSTEM_EXPLANATION.md`
- `docs/stage01_ros2_mujoco_bridge.md`
- `docs/stage02_pinocchio_kinematics_validation.md`
- `docs/stage03_standing_pd_baseline.md`
- `docs/stage04_gait_swing_foothold.md`
- `docs/stage05_standing_contact_force_qp.md`
- `docs/stage06_force_to_joint_torque.md`
- `docs/stage12_20a_source_anchor_inspection.md`
- `docs/stage12_20b_r2_source_patch_summary.md`
- `docs/stage12_20b_source_patch_summary.md`
- `docs/stage12_21_r2c_capture_failure_inspection.md`
- `docs/stage13_0_simulation_only_locomotion_preflight_inventory.md`
- `docs/stage13_1b_rerun_1200step_simulation_only_mixed_baseline.md`
- `docs/stage13_2a_2400step_preflight_and_derived_runner.md`
- `docs/stage13_2a_r1_runner_structure_inspection.md`
- `docs/stage13_2a_r2_create_2400step_derived_runner.md`
- `docs/stage13_2b_r1_derive_contact_schedule_and_rerun_2400step.md`
- `docs/stage13_2b_run_2400step_simulation_only_robustness_regression.md`
- `docs/stage13_2c_final_2400step_robustness_evidence_freeze.md`
- `docs/stage13_3_report_ready_results_packaging.md`
- `docs/stage13_4a_report_ready_plots_and_tables.md`
- `docs/stage13_4b_report_ready_package_manifest_freeze.md`
- `docs/stage13_5a_mujoco_offscreen_rollout_demo_video.md`
- `docs/stage13_5a_r1_mujoco_offscreen_rollout_video_640fb.md`
- `docs/stage13_5a_r2_mujoco_offscreen_rollout_video_fixed_ffmpeg.md`
- `docs/stage13_5b_demo_video_evidence_freeze.md`
- `docs/stage13_5c_final_package_with_demo_video_manifest.md`
- `docs/stage14_4_base_velocity_tracking_mpc.md`
- `docs/stage14_4b_base_velocity_tracking_mpc_validation.md`
- `docs/stage14_4c_mpc_scope_explanation.md`
- `docs/stage14_4d_document_language_audit.md`
- `results/logs_sample/stage08_6_backup_stage07_online_full_wbc_scheduler_recommended_run.py.txt`
- `results/logs_sample/stage08_6_backup_stage07_online_stance_pd_wbc_plus_swing_pd_recommended_test.py.txt`
- `results/logs_sample/stage08_ab_adapter_backed_stdout.txt`
- `results/logs_sample/stage08_ab_original_stdout.txt`
- `results/logs_sample/stage08_adapter_zero_control_regression_guard_stage07_stdout.txt`
- `results/logs_sample/stage08_freeze_integrity_recommended_entrypoint_stdout.txt`
- `results/logs_sample/stage08_recommended_runtime_safe_entrypoint_stdout.txt`
- `results/logs_sample/stage09_cpp_mirror_runtime_contract_guard_bridge_stderr.txt`
- `results/logs_sample/stage09_ros2_runtime_mirror_bridge_stderr.txt`
- `results/logs_sample/stage10_clamp_watchdog_bridge_stderr.txt`
- `results/logs_sample/stage10_cpp_state_cache_runtime_validation_bridge_stderr.txt`
- `results/logs_sample/stage10_disabled_controller_uses_safety_bridge_stderr.txt`
- `results/logs_sample/stage10_manual_enable_params_bridge_stderr.txt`
- `results/logs_sample/stage10_zero_torque_dry_run_bridge_stderr.txt`
- `results/logs_sample/stage11_dormant_publisher_path_source_skeleton_bridge_stderr.txt`
- `results/logs_sample/stage11_publisher_path_source_guard_bridge_stderr.txt`
- `results/logs_sample/stage11_runtime_guard_hardening_bridge_stderr.txt`
- `results/logs_sample/stage12_20a_source_anchor_inspection_summary.json`
- `results/logs_sample/stage12_20b_r1_source_structure_inspection_summary.json`
- `results/logs_sample/stage12_20b_r2_source_patch_summary.json`
- `results/logs_sample/stage12_20b_r3_repair_summary.json`
- `results/logs_sample/stage12_20b_source_patch_summary.json`
- `results/logs_sample/stage12_20c_r1_runtime_default_off_regression_summary.json`
- `results/logs_sample/stage12_20d_r1_reparse_existing_evidence_summary.json`
- `results/logs_sample/stage12_20d_runtime_bounded_streaming_regression_summary.json`
- `results/logs_sample/stage12_20e_final_evidence_freeze_summary.json`
- `results/logs_sample/stage12_21_freeze_and_regression_check_summary.json`
- `results/logs_sample/stage12_21_r1_inspect_bounded_streaming_failure_summary.json`
- `results/logs_sample/stage12_21_r2a_cleanup_and_failed_capture_inspection_summary.json`
- `results/logs_sample/stage12_21_r2b_robust_subscriber_warmup_rerun_summary.json`
- `results/logs_sample/stage12_21_r2c_capture_failure_inspection_summary.json`
- `results/logs_sample/stage12_21_r2d_repair_capture_helper_and_rerun_summary.json`
- `results/logs_sample/stage12_bounded_one_shot_publish_call_freeze_bridge_stderr.txt`
- `results/logs_sample/stage12_bounded_one_shot_publish_call_repair2_bridge_stderr.txt`
- `results/logs_sample/stage12_bounded_one_shot_publish_call_repair_bridge_stderr.txt`
- `results/logs_sample/stage12_bounded_one_shot_zero_safe_publish_call_bridge_stderr.txt`
- `results/logs_sample/stage12_manual_enable_runtime_activation_bridge_stderr.txt`
- `results/logs_sample/stage12_pre_construction_guard_bridge_stderr.txt`
- `results/logs_sample/stage12_publisher_construction_without_publish_bridge_stderr.txt`
- `results/logs_sample/stage13_0_simulation_only_locomotion_preflight_inventory_summary.json`
- `results/logs_sample/stage13_1a_existing_stage7_baseline_summary_verification_summary.json`
- `results/logs_sample/stage13_1b_rerun_1200step_simulation_only_mixed_baseline_summary.json`
- `results/logs_sample/stage13_2a_2400step_preflight_and_derived_runner_summary.json`
- `results/logs_sample/stage13_2a_r1_runner_structure_inspection_summary.json`
- `results/logs_sample/stage13_2a_r2_create_2400step_derived_runner_summary.json`
- `results/logs_sample/stage13_2b_r1_derive_contact_schedule_and_rerun_2400step_summary.json`
- `results/logs_sample/stage13_2b_run_2400step_simulation_only_robustness_regression_summary.json`
- `results/logs_sample/stage13_2c_final_2400step_robustness_evidence_freeze_summary.json`
- `results/logs_sample/stage13_3_report_ready_results_packaging_summary.json`
- `results/logs_sample/stage13_4a_report_ready_plots_and_tables_summary.json`
- `results/logs_sample/stage13_4b_report_ready_package_manifest.json`
- `results/logs_sample/stage13_4b_report_ready_package_manifest_freeze_summary.json`
- `results/logs_sample/stage13_5a_mujoco_offscreen_rollout_demo_video_summary.json`
- `results/logs_sample/stage13_5a_r1_mujoco_offscreen_rollout_video_640fb_summary.json`
- `results/logs_sample/stage13_5a_r2_mujoco_offscreen_rollout_video_fixed_ffmpeg_summary.json`
- `results/logs_sample/stage13_5b_demo_video_evidence_freeze_summary.json`
- `results/logs_sample/stage13_5c_final_package_with_demo_video_manifest.json`
- `results/logs_sample/stage13_5c_final_package_with_demo_video_manifest_summary.json`
- `results/logs_sample/stage14_2_readme_interview_entry_summary.json`
- `results/logs_sample/stage14_3_colcon_cpp_control_tests_summary.json`
- `results/logs_sample/stage14_4c_mpc_scope_explanation_summary.json`
- `ros2_ws/src/robot_mpc_wbc_bridge/launch/mujoco_bridge.launch.py`
- `ros2_ws/src/robot_mpc_wbc_bridge/robot_mpc_wbc_bridge/mujoco_bridge_node.py`
- `ros2_ws/src/robot_mpc_wbc_bridge/setup.py`
- `ros2_ws/src/robot_mpc_wbc_cpp_interface/package.xml`
- `scripts/common/__init__.py`
- `scripts/stage05_centroidal_z_mpc.py`
- `scripts/stage05_contact_schedule_force_qp.py`
- `scripts/stage05_horizon_force_qp.py`
- `scripts/stage05_standing_contact_force_qp.py`
- `scripts/stage06_qp_force_to_torque.py`
- `scripts/stage06_qp_torque_support_test.py`
- `scripts/stage07_compare_wbc_variants.py`
- `scripts/stage07_contact_schedule_wbc_qp.py`
- `scripts/stage07_contact_schedule_wbc_scale_sweep.py`
- `scripts/stage07_contact_schedule_wbc_scaled_support_test.py`
- `scripts/stage07_contact_schedule_wbc_support_test.py`
- `scripts/stage07_full_wbc_base_accel_task_qp.py`
- `scripts/stage07_full_wbc_base_vertical_accel_task_qp.py`
- `scripts/stage07_full_wbc_dynamics_qp.py`
- `scripts/stage07_full_wbc_stance_constraint_qp.py`
- `scripts/stage07_full_wbc_swing_accel_task_qp.py`
- `scripts/stage07_full_wbc_torque_ramp_check.py`
- `scripts/stage07_full_wbc_torque_reconstruction_check.py`
- `scripts/stage07_full_wbc_torque_sequence_support_test.py`
- `scripts/stage07_minimal_wbc_torque_qp.py`
- `scripts/stage07_online_full_wbc_plus_swing_joint_tracking_proto.py`
- `scripts/stage07_online_full_wbc_plus_swing_joint_tracking_sweep.py`
- `scripts/stage07_online_full_wbc_scheduler_recommended_run.py`
- `scripts/stage07_online_full_wbc_scheduler_stability_sweep.py`
- `scripts/stage07_online_full_wbc_step_loop_proto.py`
- `scripts/stage07_online_full_wbc_with_scheduler_proto.py`
- `scripts/stage07_online_stance_pd_wbc_plus_swing_pd_recommended_test.py`
- `scripts/stage07_online_stance_pd_wbc_plus_swing_pd_sweep.py`
- `scripts/stage07_online_stance_wbc_plus_swing_pd_sweep.py`
- `scripts/stage07_online_swing_trajectory_tracking_check.py`
- `scripts/stage07_swing_foot_tracking_qp.py`
- `scripts/stage07_swing_joint_target_tracking_with_wbc_test.py`
- `scripts/stage07_swing_tracking_stability_sweep.py`
- `scripts/stage07_swing_trajectory_qp.py`
- `scripts/stage07_swing_trajectory_qp_k9.py`
- `scripts/stage07_wbc_base_wrench_qp.py`
- `scripts/stage07_wbc_base_wrench_support_test.py`
- `scripts/stage07_wbc_posture_regularized_qp.py`
- `scripts/stage07_wbc_posture_regularized_support_test.py`
- `scripts/stage07_wbc_torque_support_test.py`
- `scripts/stage08_active_leg_order_refactor_and_regression.py`
- `scripts/stage08_adapter_backed_stage07_baseline_ab_test.py`
- `scripts/stage08_adapter_zero_control_regression_guard.py`
- `scripts/stage08_freeze_integrity_check.py`
- `scripts/stage08_runtime_interface_adapter_module_check.py`
- `scripts/stage08_runtime_interface_contract_check.py`
- `scripts/stage08_runtime_mapping_audit_triage.py`
- `scripts/stage08_runtime_mapping_duplication_audit.py`
- `scripts/stage09_cpp_mirror_contract_report.py`
- `scripts/stage09_cpp_mirror_runtime_contract_guard.py`
- `scripts/stage09_interface_mirror_freeze_summary.py`
- `scripts/stage09_python_baseline_ros2_field_mapping.py`
- `scripts/stage09_ros2_cpp_interface_contract_inventory.py`
- `scripts/stage09_ros2_cpp_interface_mirror_skeleton_check.py`
- `scripts/stage09_ros2_runtime_mirror_smoke_test.py`
- `scripts/stage09_ros2_topic_schema_snapshot.py`
- `scripts/stage10_0_11_full_no_publisher_controller_freeze.py`
- `scripts/stage10_0_5_controller_planning_freeze_summary.py`
- `scripts/stage10_7_8_safety_utility_freeze_summary.py`
- `scripts/stage10_clamp_watchdog_utility_without_publisher.py`
- `scripts/stage10_controller_implementation_plan_and_safety_gate.py`
- `scripts/stage10_cpp_state_cache_runtime_validation.py`
- `scripts/stage10_disabled_controller_uses_safety_utilities.py`
- `scripts/stage10_disabled_cpp_controller_skeleton_check.py`
- `scripts/stage10_manual_enable_param_guard_freeze_summary.py`
- `scripts/stage10_manual_enable_params_disabled_without_publisher.py`
- `scripts/stage10_python_frozen_baseline_ab_regression.py`
- `scripts/stage10_torque_publisher_enable_gate_design.py`
- `scripts/stage10_zero_torque_dry_run_internal_validation.py`
- `scripts/stage11_0_2_publisher_path_planning_freeze_summary.py`
- `scripts/stage11_0_8_full_publisher_path_no_active_publisher_freeze.py`
- `scripts/stage11_5_7_dormant_publisher_runtime_freeze_summary.py`
- `scripts/stage11_disabled_publisher_path_skeleton_design_only.py`
- `scripts/stage11_disabled_publisher_path_skeleton_preflight.py`
- `scripts/stage11_dormant_publisher_path_source_skeleton.py`
- `scripts/stage11_dormant_publisher_path_source_skeleton_freeze_summary.py`
- `scripts/stage11_full_freeze_integrity_check.py`
- `scripts/stage11_publisher_path_skeleton_planning_only.py`
- `scripts/stage11_publisher_path_source_guard_before_implementation.py`
- `scripts/stage11_runtime_guard_hardening_for_dormant_publisher_skeleton.py`
- `scripts/stage12_active_publisher_construction_planning_only.py`
- `scripts/stage12_bounded_one_shot_publish_call_freeze_regression.py`
- `scripts/stage12_bounded_one_shot_publish_call_repair.py`
- `scripts/stage12_bounded_one_shot_zero_safe_publish_call_source_patch.py`
- `scripts/stage12_bounded_publish_call_source_patch_design_only.py`
- `scripts/stage12_bounded_publish_call_source_patch_preflight_freeze.py`
- `scripts/stage12_bounded_zero_safe_publish_call_implementation_plan.py`
- `scripts/stage12_construction_stage_preflight_freeze.py`
- `scripts/stage12_continuous_torque_streaming_design_only.py`
- `scripts/stage12_continuous_torque_streaming_preflight_freeze.py`
- `scripts/stage12_continuous_torque_streaming_source_patch_design_only.py`
- `scripts/stage12_continuous_torque_streaming_source_patch_preflight_freeze.py`
- `scripts/stage12_manual_enable_activation_design_only.py`
- `scripts/stage12_manual_enable_no_publish_freeze.py`
- `scripts/stage12_manual_enable_runtime_activation_without_publish.py`
- `scripts/stage12_pre_construction_source_runtime_guard.py`
- `scripts/stage12_publish_call_design_only.py`
- `scripts/stage12_publish_call_preflight_freeze.py`
- `scripts/stage12_publisher_construction_no_publish_freeze.py`
- `scripts/stage12_publisher_construction_source_patch_design_only.py`
- `scripts/stage12_publisher_construction_source_patch_without_publish.py`
- `scripts/stage13_0_simulation_only_locomotion_preflight_inventory.py`
- `scripts/stage13_2_2400step_online_full_wbc_scheduler_runner.py`
- `scripts/stage13_2_2400step_simulation_only_mixed_baseline_runner.py`
- `scripts/stage13_2a_r1_runner_structure_inspection.py`
- `scripts/stage13_2a_r2_create_2400step_derived_runner.py`
- `scripts/stage13_2b_r1_derive_contact_schedule_and_rerun_2400step.py`
- `scripts/stage13_2b_run_2400step_simulation_only_robustness_regression.py`
- `scripts/stage13_2c_final_2400step_robustness_evidence_freeze.py`
- `scripts/stage13_3_report_ready_results_packaging.py`
- `scripts/stage14_4_base_velocity_tracking_mpc_demo.py`
- `scripts/stage14_4b_validate_base_velocity_mpc_rollout.py`
- `scripts/stage14_4c_validate_mpc_scope_explanation.py`
- `scripts/stage14_4d_audit_document_language.py`
- `scripts/stage14_4e_update_entry_docs.py`
- `scripts/stage14_4e_validate_entry_docs.py`

### torque_mapping

count: 205

- `PROJECT_STATUS.md`
- `README.md`
- `docs/CONTROL_ARCHITECTURE_OVERVIEW.md`
- `docs/DEMO_VIDEO_MANIFEST.md`
- `docs/FINAL_PACKAGE_WITH_DEMO_VIDEO_MANIFEST.md`
- `docs/ONE_PAGE_TECHNICAL_REPORT.md`
- `docs/REPORT_READY_FIGURES.md`
- `docs/REPORT_READY_METRICS_TABLE.md`
- `docs/REPORT_READY_PACKAGE_MANIFEST.md`
- `docs/REPORT_READY_RESULTS.md`
- `docs/SIMULATION_ONLY_RESULTS_SUMMARY.md`
- `docs/STAGE06_QP_FORCE_TO_ACTUATOR_TORQUE.md`
- `docs/STAGE06_QP_TORQUE_SUPPORT_TEST.md`
- `docs/STAGE06_SUMMARY_AND_STAGE07_INTERFACE.md`
- `docs/STAGE07_CONSOLIDATED_SUMMARY_AFTER_SWING.md`
- `docs/STAGE07_CONTACT_MODE_SEQUENCE_RAMP_TEST.md`
- `docs/STAGE07_CONTACT_MODE_TORQUE_RAMP_CHECK.md`
- `docs/STAGE07_CONTACT_SCHEDULE_WBC_QP.md`
- `docs/STAGE07_CONTACT_SCHEDULE_WBC_SCALED_SUPPORT_TEST.md`
- `docs/STAGE07_FINAL_SUMMARY.md`
- `docs/STAGE07_FINAL_UPDATE_AFTER_MODE_SEQUENCE.md`
- `docs/STAGE07_FULL_WBC_BASE_VERTICAL_ACCEL_TASK_QP.md`
- `docs/STAGE07_FULL_WBC_DYNAMICS_QP.md`
- `docs/STAGE07_FULL_WBC_FINAL_UPDATE.md`
- `docs/STAGE07_FULL_WBC_STANCE_CONSTRAINT_QP.md`
- `docs/STAGE07_FULL_WBC_SUMMARY.md`
- `docs/STAGE07_FULL_WBC_SWING_ACCEL_TASK_QP.md`
- `docs/STAGE07_FULL_WBC_TORQUE_RECONSTRUCTION_AND_RAMP_CHECK.md`
- `docs/STAGE07_FULL_WBC_TORQUE_SEQUENCE_SUPPORT_TEST.md`
- `docs/STAGE07_MINIMAL_WBC_TORQUE_QP.md`
- `docs/STAGE07_ONLINE_FULL_WBC_PLUS_SWING_JOINT_TRACKING_SWEEP.md`
- `docs/STAGE07_ONLINE_FULL_WBC_SCHEDULER_RECOMMENDED_RUN.md`
- `docs/STAGE07_ONLINE_FULL_WBC_SCHEDULER_STABILITY_SWEEP.md`
- `docs/STAGE07_ONLINE_FULL_WBC_STEP_LOOP_PROTO.md`
- `docs/STAGE07_ONLINE_FULL_WBC_WITH_SCHEDULER_PROTO.md`
- `docs/STAGE07_ONLINE_LOCOMOTION_CONSOLIDATED_SUMMARY.md`
- `docs/STAGE07_ONLINE_STANCE_PD_WBC_PLUS_SWING_PD_RECOMMENDED_TEST.md`
- `docs/STAGE07_ONLINE_STANCE_PD_WBC_PLUS_SWING_PD_SWEEP.md`
- `docs/STAGE07_ONLINE_SWING_JOINT_TRACKING_RECOMMENDED_TEST.md`
- `docs/STAGE07_ONLINE_SWING_JOINT_TRACKING_STABILITY_SWEEP.md`
- `docs/STAGE07_ONLINE_SWING_TRAJECTORY_MEMORY_PROTO.md`
- `docs/STAGE07_ONLINE_SWING_TRAJECTORY_TRACKING_CHECK.md`
- `docs/STAGE07_SUMMARY_AND_NEXT_STEPS.md`
- `docs/STAGE07_SWING_FOOT_TRACKING_QP.md`
- `docs/STAGE07_SWING_JOINT_TARGET_TRACKING_RECOMMENDED_TEST.md`
- `docs/STAGE07_SWING_TRACKING_MODE_SEQUENCE_TEST.md`
- `docs/STAGE07_SWING_TRACKING_RECOMMENDED_BOTH_MODES.md`
- `docs/STAGE07_SWING_TRAJECTORY_QP.md`
- `docs/STAGE07_WBC_BASE_WRENCH_QP.md`
- `docs/STAGE07_WBC_BASE_WRENCH_SUPPORT_TEST.md`
- `docs/STAGE07_WBC_TORQUE_SUPPORT_TEST.md`
- `docs/STAGE07_WBC_VARIANT_COMPARISON.md`
- `docs/STAGE08_0_7_RUNTIME_INTERFACE_HARDENING_SUMMARY.md`
- `docs/STAGE08_ADAPTER_ZERO_CONTROL_REGRESSION_GUARD.md`
- `docs/STAGE08_RUNTIME_INTERFACE_ADAPTER_MODULE_CHECK.md`
- `docs/STAGE08_RUNTIME_INTERFACE_CONTRACT_CHECK.md`
- `docs/STAGE08_RUNTIME_MAPPING_AUDIT_TRIAGE.md`
- `docs/STAGE08_RUNTIME_MAPPING_DUPLICATION_AUDIT.md`
- `docs/STAGE09_PYTHON_BASELINE_ROS2_FIELD_MAPPING.md`
- `docs/WBC_QP_EXPLAINED.md`
- `docs/interview/INTERVIEW_3MIN_SYSTEM_EXPLANATION.md`
- `docs/stage02_pinocchio_kinematics_validation.md`
- `docs/stage03_standing_pd_baseline.md`
- `docs/stage06_force_to_joint_torque.md`
- `docs/stage12_20b_r1_source_structure_inspection.md`
- `docs/stage13_0_simulation_only_locomotion_preflight_inventory.md`
- `docs/stage13_1a_existing_stage7_baseline_summary_verification.md`
- `docs/stage13_1b_rerun_1200step_simulation_only_mixed_baseline.md`
- `docs/stage13_2b_r1_derive_contact_schedule_and_rerun_2400step.md`
- `docs/stage13_2c_final_2400step_robustness_evidence_freeze.md`
- `docs/stage13_4a_report_ready_plots_and_tables.md`
- `docs/stage13_5b_demo_video_evidence_freeze.md`
- `results/logs_sample/stage08_6_backup_stage07_online_full_wbc_scheduler_recommended_run.py.txt`
- `results/logs_sample/stage08_6_backup_stage07_online_stance_pd_wbc_plus_swing_pd_recommended_test.py.txt`
- `results/logs_sample/stage08_ab_adapter_backed_stdout.txt`
- `results/logs_sample/stage08_ab_original_stdout.txt`
- `results/logs_sample/stage08_adapter_zero_control_regression_guard_stage07_stdout.txt`
- `results/logs_sample/stage08_freeze_integrity_recommended_entrypoint_stdout.txt`
- `results/logs_sample/stage08_recommended_runtime_safe_entrypoint_stdout.txt`
- `results/logs_sample/stage10_clamp_watchdog_contract_check_stdout.txt`
- `results/logs_sample/stage10_disabled_controller_uses_safety_contract_stdout.txt`
- `results/logs_sample/stage10_disabled_controller_uses_safety_controller_stderr.txt`
- `results/logs_sample/stage10_manual_enable_params_controller_stderr.txt`
- `results/logs_sample/stage10_zero_torque_dry_run_contract_check_stdout.txt`
- `results/logs_sample/stage11_dormant_publisher_path_source_skeleton_controller_stderr.txt`
- `results/logs_sample/stage11_publisher_path_source_guard_controller_stderr.txt`
- `results/logs_sample/stage11_runtime_guard_hardening_controller_stderr.txt`
- `results/logs_sample/stage12_20a_source_anchor_inspection_summary.json`
- `results/logs_sample/stage12_20b_r1_source_structure_inspection_summary.json`
- `results/logs_sample/stage12_bounded_one_shot_publish_call_freeze_default_controller_stderr.txt`
- `results/logs_sample/stage12_bounded_one_shot_publish_call_freeze_enabled_controller_stderr.txt`
- `results/logs_sample/stage12_bounded_one_shot_publish_call_repair2_controller_stderr.txt`
- `results/logs_sample/stage12_bounded_one_shot_publish_call_repair_controller_stderr.txt`
- `results/logs_sample/stage12_bounded_one_shot_zero_safe_publish_call_controller_stderr.txt`
- `results/logs_sample/stage12_disabled_controller_node_after_stage1214.cpp`
- `results/logs_sample/stage12_disabled_controller_node_after_stage1214_repair.cpp`
- `results/logs_sample/stage12_disabled_controller_node_after_stage1214_repair2.cpp`
- `results/logs_sample/stage12_disabled_controller_node_after_stage124.cpp`
- `results/logs_sample/stage12_disabled_controller_node_before_stage1214.cpp`
- `results/logs_sample/stage12_disabled_controller_node_before_stage1214_repair.cpp`
- `results/logs_sample/stage12_disabled_controller_node_before_stage1214_repair2.cpp`
- `results/logs_sample/stage12_disabled_controller_node_before_stage124.cpp`
- `results/logs_sample/stage12_manual_enable_runtime_activation_controller_stderr.txt`
- `results/logs_sample/stage12_pre_construction_guard_controller_stderr.txt`
- `results/logs_sample/stage12_publisher_construction_without_publish_controller_stderr.txt`
- `results/logs_sample/stage13_0_simulation_only_locomotion_preflight_inventory_summary.json`
- `results/logs_sample/stage13_1a_existing_stage7_baseline_summary_verification_summary.json`
- `results/logs_sample/stage13_1b_rerun_1200step_simulation_only_mixed_baseline_summary.json`
- `results/logs_sample/stage13_2b_r1_derive_contact_schedule_and_rerun_2400step_summary.json`
- `results/logs_sample/stage13_2c_final_2400step_robustness_evidence_freeze_summary.json`
- `results/logs_sample/stage13_3_report_ready_results_packaging_summary.json`
- `results/logs_sample/stage13_4a_report_ready_plots_and_tables_summary.json`
- `results/logs_sample/stage13_4b_report_ready_package_manifest.json`
- `results/logs_sample/stage13_4b_report_ready_package_manifest_freeze_summary.json`
- `results/logs_sample/stage13_5a_r2_mujoco_offscreen_rollout_video_fixed_ffmpeg_summary.json`
- `results/logs_sample/stage13_5b_demo_video_evidence_freeze_summary.json`
- `results/logs_sample/stage13_5c_final_package_with_demo_video_manifest.json`
- `results/logs_sample/stage13_5c_final_package_with_demo_video_manifest_summary.json`
- `ros2_ws/src/robot_mpc_wbc_cpp_controller/include/robot_mpc_wbc_cpp_controller/control/torque_safety_filter.hpp`
- `ros2_ws/src/robot_mpc_wbc_cpp_controller/include/robot_mpc_wbc_cpp_controller/torque_safety.hpp`
- `ros2_ws/src/robot_mpc_wbc_cpp_controller/include/robot_mpc_wbc_cpp_controller/zero_torque_dry_run.hpp`
- `ros2_ws/src/robot_mpc_wbc_cpp_controller/src/control/torque_safety_filter.cpp`
- `ros2_ws/src/robot_mpc_wbc_cpp_controller/src/disabled_controller_node.cpp`
- `ros2_ws/src/robot_mpc_wbc_cpp_controller/src/torque_safety_contract_check.cpp`
- `ros2_ws/src/robot_mpc_wbc_cpp_controller/src/zero_torque_dry_run_contract_check.cpp`
- `ros2_ws/src/robot_mpc_wbc_cpp_controller/test/test_control_algorithms.cpp`
- `scripts/common/go1_runtime_interface.py`
- `scripts/stage02_check_pinocchio_jacobian.py`
- `scripts/stage02_compare_foot_velocity.py`
- `scripts/stage02_compare_full_foot_velocity.py`
- `scripts/stage02_compare_joint_order.py`
- `scripts/stage02_compare_mujoco_pinocchio_fk.py`
- `scripts/stage02_compare_mujoco_pinocchio_jacobian.py`
- `scripts/stage02_inspect_pinocchio_model.py`
- `scripts/stage02_validate_pinocchio_fk.py`
- `scripts/stage03_standing_pd_demo.py`
- `scripts/stage04_open_loop_trot_pd_demo.py`
- `scripts/stage06_force_sign_sanity_check.py`
- `scripts/stage06_jacobian_transpose_torque.py`
- `scripts/stage06_qp_force_to_torque.py`
- `scripts/stage06_qp_torque_support_test.py`
- `scripts/stage07_compare_wbc_variants.py`
- `scripts/stage07_contact_mode_sequence_ramp_test.py`
- `scripts/stage07_contact_mode_torque_ramp_check.py`
- `scripts/stage07_contact_mode_transition_check.py`
- `scripts/stage07_contact_schedule_wbc_qp.py`
- `scripts/stage07_contact_schedule_wbc_scale_sweep.py`
- `scripts/stage07_contact_schedule_wbc_scaled_support_test.py`
- `scripts/stage07_contact_schedule_wbc_support_test.py`
- `scripts/stage07_full_wbc_base_accel_task_qp.py`
- `scripts/stage07_full_wbc_base_vertical_accel_task_qp.py`
- `scripts/stage07_full_wbc_dynamics_qp.py`
- `scripts/stage07_full_wbc_stance_constraint_qp.py`
- `scripts/stage07_full_wbc_swing_accel_task_qp.py`
- `scripts/stage07_full_wbc_torque_ramp_check.py`
- `scripts/stage07_full_wbc_torque_reconstruction_check.py`
- `scripts/stage07_full_wbc_torque_sequence_support_test.py`
- `scripts/stage07_minimal_wbc_torque_qp.py`
- `scripts/stage07_online_full_wbc_plus_swing_joint_tracking_proto.py`
- `scripts/stage07_online_full_wbc_plus_swing_joint_tracking_sweep.py`
- `scripts/stage07_online_full_wbc_scheduler_recommended_run.py`
- `scripts/stage07_online_full_wbc_scheduler_stability_sweep.py`
- `scripts/stage07_online_full_wbc_step_loop_proto.py`
- `scripts/stage07_online_full_wbc_with_scheduler_proto.py`
- `scripts/stage07_online_stance_pd_wbc_plus_swing_pd_recommended_test.py`
- `scripts/stage07_online_stance_pd_wbc_plus_swing_pd_sweep.py`
- `scripts/stage07_online_stance_wbc_plus_swing_pd_sweep.py`
- `scripts/stage07_online_swing_joint_target_tracking_support_test.py`
- `scripts/stage07_online_swing_joint_tracking_recommended_test.py`
- `scripts/stage07_online_swing_joint_tracking_stability_sweep.py`
- `scripts/stage07_online_swing_trajectory_tracking_check.py`
- `scripts/stage07_swing_joint_target_tracking_recommended_test.py`
- `scripts/stage07_swing_joint_target_tracking_recommended_test_flrr.py`
- `scripts/stage07_swing_joint_target_tracking_test.py`
- `scripts/stage07_swing_joint_target_tracking_with_wbc_test.py`
- `scripts/stage07_swing_tracking_mode_sequence_test.py`
- `scripts/stage07_swing_tracking_stability_sweep.py`
- `scripts/stage07_wbc_base_wrench_qp.py`
- `scripts/stage07_wbc_base_wrench_support_test.py`
- `scripts/stage07_wbc_posture_regularized_qp.py`
- `scripts/stage07_wbc_posture_regularized_support_test.py`
- `scripts/stage07_wbc_torque_support_test.py`
- `scripts/stage08_adapter_backed_stage07_baseline_ab_test.py`
- `scripts/stage08_adapter_backed_stage07_recommended_test.py`
- `scripts/stage08_adapter_zero_control_regression_guard.py`
- `scripts/stage08_runtime_interface_adapter_module_check.py`
- `scripts/stage08_runtime_interface_contract_check.py`
- `scripts/stage08_runtime_mapping_duplication_audit.py`
- `scripts/stage09_python_baseline_ros2_field_mapping.py`
- `scripts/stage10_7_8_safety_utility_freeze_summary.py`
- `scripts/stage10_clamp_watchdog_utility_without_publisher.py`
- `scripts/stage10_disabled_controller_uses_safety_utilities.py`
- `scripts/stage10_zero_torque_dry_run_internal_validation.py`
- `scripts/stage13_0_simulation_only_locomotion_preflight_inventory.py`
- `scripts/stage13_1a_existing_stage7_baseline_summary_verification.py`
- `scripts/stage13_1b_rerun_1200step_simulation_only_mixed_baseline.py`
- `scripts/stage13_2_2400step_online_full_wbc_scheduler_runner.py`
- `scripts/stage13_2_2400step_simulation_only_mixed_baseline_runner.py`
- `scripts/stage13_2a_2400step_preflight_and_derived_runner.py`
- `scripts/stage13_2b_r1_derive_contact_schedule_and_rerun_2400step.py`
- `scripts/stage13_2b_run_2400step_simulation_only_robustness_regression.py`
- `scripts/stage13_2c_final_2400step_robustness_evidence_freeze.py`
- `scripts/stage13_3_report_ready_results_packaging.py`
- `scripts/stage13_4a_report_ready_plots_and_tables.py`
- `scripts/stage13_5b_demo_video_evidence_freeze.py`

## Boundary

The output of this stage is an interface/file inventory only. It is not MPC-WBC integration evidence, not closed-loop locomotion evidence, not hardware readiness evidence, and not real robot torque execution evidence.
