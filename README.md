...
项目的核心亮点是：没有直接将 MPC/WBC 候选力矩表述为稳定主控结论，而是先记录 `primary_mpc_wbc` 直接主控模式的失败，再通过失败诊断引入斜坡过渡、尺度限制、站立腿姿态残差和在线 WBC 残差，最终得到 `stabilized_primary_mpc_wbc` 在固定仿真场景下的闭环运行证据。
