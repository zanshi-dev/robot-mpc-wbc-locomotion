...
OUTPUT_CSV = "results/logs_sample/stage07_wbc_posture_regularized_qp.csv"
...
def build_posture_matrix():
    ...
    # 左右 hip torque 反对称：FR + FL -> 0, RR + RL -> 0
    D[0, 0] = 1.0
    D[0, 3] = 1.0
    D[1, 6] = 1.0
    D[1, 9] = 1.0

    # 前后同侧 hip torque 接近：FR - RR -> 0, FL - RL -> 0
    D[2, 0] = 1.0
    D[2, 6] = -1.0
    D[3, 3] = 1.0
    D[3, 9] = -1.0

    # 前后 calf torque 接近：FR - RR -> 0, FL - RL -> 0
    D[4, 2] = 1.0
    D[4, 8] = -1.0
    D[5, 5] = 1.0
    D[5, 11] = -1.0
...
def solve_qp(A_wrench, B_tau, D_posture, f_ref, tau_ref):
...
def main():
    ...
    D_posture = build_posture_matrix()

    result, desired_wrench = solve_qp(
        A_wrench=A_wrench,
        B_tau=B_tau,
        D_posture=D_posture,
        f_ref=f_ref,
    ...
    posture_ref = D_posture @ tau_ref
    posture_wbc = D_posture @ tau_wbc
    posture_error = posture_wbc
    ...
    posture_norm_ref = float(np.linalg.norm(posture_ref))
    posture_norm_wbc = float(np.linalg.norm(posture_wbc))
    posture_reduction = posture_norm_ref - posture_norm_wbc
    ...
    row = {
        ...
        "posture_norm_ref": f"{posture_norm_ref:.12f}",
        "posture_norm_wbc": f"{posture_norm_wbc:.12f}",
        "posture_reduction": f"{posture_reduction:.12f}",
    ...
    for i, value in enumerate(posture_ref):
        row[f"posture_ref_{i}"] = f"{value:.12f}"
    for i, value in enumerate(posture_wbc):
        row[f"posture_wbc_{i}"] = f"{value:.12f}"
    for i, value in enumerate(posture_error):
        row[f"posture_error_{i}"] = f"{value:.12f}"
    ...
    print("Stage 7 WBC posture regularized QP")
    for k in [
        "osqp_status",
        "tau_max_abs",
        "min_fz",
