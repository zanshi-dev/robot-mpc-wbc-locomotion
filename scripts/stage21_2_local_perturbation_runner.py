...
LOG_CSV_TEMPLATE = "results/logs_sample/stage21_2_local_perturb_{perturbation_id}_{scale_tag}_{control_mode}_log.csv"
SUMMARY_CSV_TEMPLATE = "results/logs_sample/stage21_2_local_perturb_{perturbation_id}_{scale_tag}_{control_mode}_summary.csv"
...
def _apply_initial_perturbation(model, data, perturb_x, perturb_y, perturb_yaw):
    if len(data.qpos) >= 2:
        data.qpos[0] += perturb_x
        data.qpos[1] += perturb_y

    if abs(perturb_yaw) > 0.0 and len(data.qpos) >= 7:
        current = [float(data.qpos[3]), float(data.qpos[4]), float(data.qpos[5]), float(data.qpos[6])]
        half = 0.5 * perturb_yaw
        delta = [math.cos(half), 0.0, 0.0, math.sin(half)]
        updated = _quat_normalize(_quat_mul(delta, current))
        data.qpos[3:7] = updated
...
def main():
    ...
    parser.add_argument("--perturb-x", type=float, default=0.0)
    parser.add_argument("--perturb-y", type=float, default=0.0)
    parser.add_argument("--perturb-yaw", type=float, default=0.0)
