...
OUTPUT_CSV = "results/logs_sample/stage07_full_wbc_base_accel_task_qp.csv"
...
# base qdd task：MuJoCo free joint qvel 前 3 维为 base linear，后 3 维为 base angular
BASE_QDD_REF = np.array([0.0, 0.0, 0.2, 0.0, 0.0, 0.0], dtype=float)
...
def solve_mode(model, data, site_ids, dofs, row, mode):
    ...
    weights[qdd_off:qdd_off + 6] = W_BASE_TASK
    x_ref[qdd_off:qdd_off + 6] = BASE_QDD_REF
    ...
    # standing pose 且 qvel=0，因此 Jdot_v = 0
    ...
    base_task_error = qdd[:6] - BASE_QDD_REF
    ...
    base_task_error_norm = float(np.linalg.norm(base_task_error))
    base_task_error_max_abs = float(np.max(np.abs(base_task_error)))
    ...
    base_task_pass = base_task_error_norm < BASE_TASK_ERROR_TOL
    ...
    out = {
        ...
        "base_qdd_ref_x": f"{BASE_QDD_REF[0]:.12f}",
        "base_qdd_ref_y": f"{BASE_QDD_REF[1]:.12f}",
        "base_qdd_ref_z": f"{BASE_QDD_REF[2]:.12f}",
        "base_qdd_ref_roll": f"{BASE_QDD_REF[3]:.12f}",
        "base_qdd_ref_pitch": f"{BASE_QDD_REF[4]:.12f}",
        "base_qdd_ref_yaw": f"{BASE_QDD_REF[5]:.12f}",
        ...
        "base_task_error_norm": f"{base_task_error_norm:.12e}",
        "base_task_error_max_abs": f"{base_task_error_max_abs:.12e}",
        ...
        "base_task_pass": str(base_task_pass),
...
def main():
    ...
    print("Stage 7 full WBC base accel task QP")
    ...
    for row in rows:
        print(
            ...
            f"base_task_error_norm={row.get('base_task_error_norm', 'nan')} "
