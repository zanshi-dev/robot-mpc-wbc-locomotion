...
def main() -> int:
    ...
    execution_summary_path = logs / "stage25_6_stabilized_primary_mpc_wbc_smoke_execution_summary.json"
    ...
    validation_csv = logs / "stage25_6_stabilized_primary_mpc_wbc_smoke_validation.csv"
    summary_json = logs / "stage25_6_stabilized_primary_mpc_wbc_smoke_summary.json"
    doc = docs / "STAGE25_6_STABILIZED_PRIMARY_MPC_WBC_SMOKE_ROLLOUT.md"
    ...
    check("stage25_5_summary_exists", s25_5_summary_path.is_file() and s25_5_summary_path.stat().st_size > 0, str(s25_5_summary_path.relative_to(root)))
    check("stage25_5_result_pass", s25_5.get("result") == "pass", f"result={s25_5.get('result')}")
    check("stage25_5_stabilized_implemented", s25_5.get("stabilized_primary_mpc_wbc_mode_implemented") is True, str(s25_5.get("stabilized_primary_mpc_wbc_mode_implemented")))
    ...
    check("stabilized_primary_mpc_wbc_executed", exe.get("stabilized_primary_mpc_wbc_executed") is True, str(exe.get("stabilized_primary_mpc_wbc_executed")))
    ...
    required_fields = [
        "control_mode",
        "stabilized_primary_mpc_wbc_mode_present",
        "stabilized_primary_mpc_wbc_executed",
        "stabilized_primary_mpc_wbc_simulation_only",
        "candidate_available_in_run",
        "stabilized_primary_scale",
        "stabilized_primary_ramp_steps",
    ...
    summary = {
        "stage": "25.6",
        "name": "stabilized_primary_mpc_wbc smoke rollout validation",
        "result": result,
        "failure_count": failure_count,
        "stabilized_primary_mpc_wbc_executed": exe.get("stabilized_primary_mpc_wbc_executed"),
    ...
    doc.write_text(f"""# Stage 25.6：stabilized_primary_mpc_wbc smoke rollout
...
""", encoding="utf-8")
    ...
    print(f"stage25_6_result: {result}")
    print(f"failure_count: {failure_count}")
    print(f"stabilized_primary_mpc_wbc_executed: {exe.get('stabilized_primary_mpc_wbc_executed')}")
