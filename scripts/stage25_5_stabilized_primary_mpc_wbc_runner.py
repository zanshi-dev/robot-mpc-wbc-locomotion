...
LOG_CSV_TEMPLATE = "results/logs_sample/stage25_5_stabilized_primary_mpc_wbc_rollout_{perturbation_id}_{scale_tag}_{control_mode}_log.csv"
SUMMARY_CSV_TEMPLATE = "results/logs_sample/stage25_5_stabilized_primary_mpc_wbc_rollout_{perturbation_id}_{scale_tag}_{control_mode}_summary.csv"
...
_LAST_TRACE_QPOS = None
_LAST_TRACE_TIME = None


def _safe_arr(data, attr, idx):
    arr = getattr(data, attr)
    return float(arr[idx]) if len(arr) > idx else 0.0


def _trace_snapshot(phase, trace_step_index, model, data):
    global _LAST_TRACE_QPOS, _LAST_TRACE_TIME

    qpos0 = _safe_arr(data, "qpos", 0)
    qpos1 = _safe_arr(data, "qpos", 1)
    qpos2 = _safe_arr(data, "qpos", 2)
    qvel0 = _safe_arr(data, "qvel", 0)
    qvel1 = _safe_arr(data, "qvel", 1)
    qvel2 = _safe_arr(data, "qvel", 2)
    qvel3 = _safe_arr(data, "qvel", 3)
    qvel4 = _safe_arr(data, "qvel", 4)
    qvel5 = _safe_arr(data, "qvel", 5)
    time_now = float(data.time)

    if _LAST_TRACE_QPOS is not None and _LAST_TRACE_TIME is not None and time_now > _LAST_TRACE_TIME:
        dt = time_now - _LAST_TRACE_TIME
        base_vx_fd = (qpos0 - _LAST_TRACE_QPOS[0]) / dt
        base_vy_fd = (qpos1 - _LAST_TRACE_QPOS[1]) / dt
        base_vz_fd = (qpos2 - _LAST_TRACE_QPOS[2]) / dt
    else:
        dt = 0.0
        base_vx_fd = 0.0
        base_vy_fd = 0.0
    ...
    row = {
        **TRACE_META,
        ...
        "time": time_now,
        "dt_from_previous_trace": dt,
        "qpos_size": int(len(data.qpos)),
        "qvel_size": int(len(data.qvel)),
        "qpos_0": qpos0,
        "qpos_1": qpos1,
        "qpos_2": qpos2,
        "qpos_3": _safe_arr(data, "qpos", 3),
        "qpos_4": _safe_arr(data, "qpos", 4),
        "qpos_5": _safe_arr(data, "qpos", 5),
        "qpos_6": _safe_arr(data, "qpos", 6),
        "qvel_0": qvel0,
        "qvel_1": qvel1,
        "qvel_2": qvel2,
        "qvel_3": qvel3,
        ...
        "base_y": qpos1,
        "base_z": qpos2,
        "base_vx_fd": base_vx_fd,
        "base_vy_fd": base_vy_fd,
        "base_vz_fd": base_vz_fd,
    }
    TRACE_ROWS.append(row)

    _LAST_TRACE_QPOS = [qpos0, qpos1, qpos2]
    _LAST_TRACE_TIME = time_now


def _trace_after_mj_step(model, data):
    global TRACE_STEP_COUNTER
    TRACE_STEP_COUNTER += 1
    if TRACE_STEP_COUNTER <= TRACE_STEPS:
        _trace_snapshot("after_mj_step", TRACE_STEP_COUNTER, model, data)


def _write_trace_csv(path_str):
    if not path_str:
        return
    path = Path(path_str)
    ...
    keys = []
    for k in preferred:
        if any(k in row for row in TRACE_ROWS):
            keys.append(k)
...
def _apply_initial_qvelrate_perturbation(model, data, perturb_vx, perturb_vy, perturb_yawrate):
    _trace_snapshot("before_injection", -3, model, data)
    ...
    _trace_snapshot("after_injection", -2, model, data)

    mujoco.mj_forward(model, data)

    _trace_snapshot("after_mj_forward", -1, model, data)

def main():
    parser = argparse.ArgumentParser(description="Stage 14.5D-R2 simulation-only A/B runner skeleton")
    parser.add_argument(
        "--control-mode",
        choices=[CONTROL_MODE_BASELINE, CONTROL_MODE_MPC_ASSISTED_CANDIDATE, CONTROL_MODE_PRIMARY_MPC_WBC, CONTROL_MODE_STABILIZED_PRIMARY_MPC_WBC],
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.2,
        help="Target forward velocity used only for Stage 18 velocity evidence metrics.",
    )
    parser.add_argument(
        "--scale-tag",
        type=str,
        default=None,
        help="Scale tag used to make Stage 19 sweep output filenames unique, e.g. 0p005.",
    )
    parser.add_argument(
        "--run-id",
        type=str,
        required=True,
        help="Replay run id for Stage 20 reproducibility audit, e.g. run_00.",
    )
    parser.add_argument("--perturbation-id", type=str, required=True)
    parser.add_argument("--perturbation-type", type=str, default="none")
    parser.add_argument("--perturb-vx", type=float, default=0.0)
    parser.add_argument("--perturb-vy", type=float, default=0.0)
    parser.add_argument("--perturb-yawrate", type=float, default=0.0)
    parser.add_argument("--trace-csv", type=str, default="")
    parser.add_argument("--trace-steps", type=int, default=12)
    args = parser.parse_args()

    global TRACE_META, TRACE_STEPS
    TRACE_STEPS = int(args.trace_steps)
    TRACE_META = {
        "trace_case_id": args.run_id,
        "perturbation_id": args.perturbation_id,
        "perturbation_type": args.perturbation_type,
        "perturb_vx": float(args.perturb_vx),
        "perturb_vy": float(args.perturb_vy),
        "perturb_yawrate": float(args.perturb_yawrate),
        "scale": float(args.mpc_assisted_candidate_scale),
        "scale_tag": args.scale_tag,
        "control_mode": args.control_mode,
    }

    import atexit
    atexit.register(lambda: _write_trace_csv(args.trace_csv))

    candidate_rows = []
    candidate_lookup = {}
    candidate_step_values = []

    if args.scale_tag is None:
        args.scale_tag = f"{args.mpc_assisted_candidate_scale:.3f}".replace(".", "p")
    ...
    log_csv = LOG_CSV_TEMPLATE.format(perturbation_id=args.perturbation_id, scale_tag=args.scale_tag, control_mode=args.control_mode)
    summary_csv = SUMMARY_CSV_TEMPLATE.format(perturbation_id=args.perturbation_id, scale_tag=args.scale_tag, control_mode=args.control_mode)
    ...
    _apply_initial_qvelrate_perturbation(
        model,
        data,
        args.perturb_vx,
        args.perturb_vy,
    ...
    target_vx = float(args.target_vx)
    dt = float(model.opt.timestep)
    prev_base_x = initial_x
    prev_base_y = initial_y
    base_vx_fd_values = []
    abs_velocity_error_values = []
    min_z = initial_z
    max_z = initial_z
    max_abs_roll = 0.0
    max_abs_pitch = 0.0
    ...
    for step in range(total_steps):
        ...
        tau_baseline_raw = tau_stance_pd + tau_stance_wbc + tau_swing_pd
        ...
        _trace_after_mj_step(model, data)
        ...
        if step == 0:
            ...
            base_vx_fd = (base_x - prev_base_x) / dt
            base_vy_fd = (base_y - prev_base_y) / dt
        base_vx_qvel = float(data.qvel[0]) if data.qvel.shape[0] > 0 else float("nan")
        velocity_error = base_vx_fd - target_vx
        base_vx_fd_values.append(base_vx_fd)
        abs_velocity_error_values.append(abs(velocity_error))
        prev_base_x = base_x
        prev_base_y = base_y
        ...
        log_rows.append({
            ...
            "time_s": f"{(step + 1) * dt:.12f}",
            "base_x": f"{base_x:.12f}",
            "base_y": f"{base_y:.12f}",
            "base_vx_fd": f"{base_vx_fd:.12f}",
            "base_vy_fd": f"{base_vy_fd:.12f}",
            "base_vx_qvel": f"{base_vx_qvel:.12f}",
            "target_vx": f"{target_vx:.12f}",
            "velocity_error": f"{velocity_error:.12f}",
            "base_z": f"{base_z:.12f}",
            "roll": f"{roll:.12f}",
            "pitch": f"{pitch:.12f}",
            ...
            "perturb_vy": args.perturb_vy,
            "perturb_yawrate": args.perturb_yawrate,
        "perturbation_id": args.perturbation_id,
        "perturbation_type": args.perturbation_type,
        "perturb_vx": args.perturb_vx,
        "perturb_vy": args.perturb_vy,
    ...
    lateral_displacement = final_y - initial_y
    mean_vx = float(np.mean(base_vx_fd_values)) if base_vx_fd_values else 0.0
    mean_abs_velocity_error = float(np.mean(abs_velocity_error_values)) if abs_velocity_error_values else 0.0
    max_abs_velocity_error = float(np.max(abs_velocity_error_values)) if abs_velocity_error_values else 0.0
    ...
    summary = {
        ...
        "target_vx": f"{target_vx:.12f}",
        "initial_x": f"{initial_x:.12f}",
        "final_x": f"{final_x:.12f}",
        "forward_displacement": f"{forward_displacement:.12f}",
        "initial_y": f"{initial_y:.12f}",
        "final_y": f"{final_y:.12f}",
        "lateral_displacement": f"{lateral_displacement:.12f}",
        "mean_vx": f"{mean_vx:.12f}",
        "mean_abs_velocity_error": f"{mean_abs_velocity_error:.12f}",
        "max_abs_velocity_error": f"{max_abs_velocity_error:.12f}",
        "control_mode": args.control_mode,
        "simulation_only_project": True,
        "hardware_deployment_completed": False,
        "torque_enable_ready": False,
        "torque_publisher_enabled": False,
        "control_law_changed": args.control_mode != CONTROL_MODE_BASELINE,
        ...
        "max_tau_baseline_raw_abs": f"{max_tau_baseline_raw_abs:.12f}",
