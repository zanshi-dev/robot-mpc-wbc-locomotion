...
OUTPUT_CSV = "results/logs_sample/stage07_full_wbc_dynamics_qp.csv"
...
def parse_force_ref_or_fallback(model, data, row, mode):
    f = np.zeros(12)
...
def solve_mode(model, data, site_ids, dofs, row, mode, contact_sign):
    nv = model.nv
    nu = model.nu
    nf = 12
    nx = nv + nf + nu
    ...
    f_ref = parse_force_ref_or_fallback(model, data, row, mode)
    ...
    weights = np.zeros(nx)
    weights[qdd_off:qdd_off + 6] = W_QDD_BASE
    ...
    # Dynamics:
    # M qdd + bias = S^T tau + contact_sign * J^T f
    # M qdd - contact_sign * J^T f - S^T tau = -bias
    A_dyn = np.zeros((nv, nx))
    A_dyn[:, qdd_off:qdd_off + nv] = M
    A_dyn[:, f_off:f_off + nf] = -contact_sign * J.T
    ...
    dyn_res = M @ qdd + bias - S_T @ tau - contact_sign * J.T @ f
...
def score_candidate(row):
    ...
    return (
        1,
        -float(row["qdd_norm"])
        - 0.1 * float(row["tau_diff_norm"])
        - 0.01 * float(row["force_diff_norm"]),
    )
...
def main():
    ...
    all_pass = all(r["pass"] == "True" for r in selected_rows)
    ...
    print("Stage 7 full floating-base WBC dynamics QP")
    ...
    for row in selected_rows:
        print(
            f"mode={row['mode']} "
            f"contact_sign={row['contact_sign']} "
            ...
            f"inactive_force_norm={row.get('inactive_force_norm', 'nan')} "
            f"tau_diff_norm={row.get('tau_diff_norm', 'nan')} "
            f"force_diff_norm={row.get('force_diff_norm', 'nan')} "
            f"pass={row['pass']}"
        )
    ...
    if not all_pass:
        ...
        for row in candidate_rows:
            print(
                f"candidate mode={row['mode']} "
                f"sign={row['contact_sign']} "
                f"status={row['osqp_status']} "
                f"pass={row['pass']} "
