...
SUMMARY_PATH = Path("results/logs_sample/stage15_4_pinocchio_jacobian_candidate_rollout_summary.json")
ROLLOUT_CSV_PATH = Path("results/logs_sample/stage15_4_pinocchio_jacobian_candidate_rollout.csv")
SOURCE_SCRIPT_PATH = Path("scripts/stage15_4_pinocchio_jacobian_candidate_rollout.py")
VALIDATION_SUMMARY_PATH = Path("results/logs_sample/stage15_4_pinocchio_jacobian_candidate_rollout_validation_summary.json")
VALIDATION_CSV_PATH = Path("results/logs_sample/stage15_4_pinocchio_jacobian_candidate_rollout_validation.csv")
...
def required_columns() -> List[str]:
    ...
    for i in range(12):
        columns.append(f"tau_pinocchio_candidate_{i:02d}")
...
def validate_source_contract(source_text: str) -> List[str]:
    required = [
        ...
        "build_synthetic_go1_like_pinocchio_model",
        "finite_difference_foot_jacobians",
        "pinocchio_jacobian_transpose_force_map",
...
def validate() -> Dict:
    ...
    if summary.get("pass") is not True:
        fail("stage15_4 summary pass must be true")

    for key in [
        "simulation_only_project",
        "pinocchio_imported",
        "pinocchio_jacobian_used",
        "synthetic_kinematic_model",
    ]:
    ...
    if int(summary.get("pinocchio_model_nv", -1)) != 12:
        fail("Pinocchio audit model must have nv=12")
    if int(summary.get("pinocchio_model_nq", -1)) != 12:
        fail("Pinocchio audit model must have nq=12")
    if max(float(v) for v in summary.get("foot_jacobian_norms", [0.0])) <= 1e-6:
        fail("foot Jacobian norms must be nonzero")
    ...
    numeric_columns = [c for c in required_columns() if c != "contact_mode"]
    ...
    validation_summary = {
        "stage": "15.4_validation",
        "description": "independent validation for Stage 15.4 Pinocchio Jacobian candidate rollout",
