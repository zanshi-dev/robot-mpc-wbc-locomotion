...
LOG_CSV_TEMPLATE = "results/logs_sample/stage22_2_observable_perturb_{perturbation_id}_{scale_tag}_{control_mode}_log.csv"
SUMMARY_CSV_TEMPLATE = "results/logs_sample/stage22_2_observable_perturb_{perturbation_id}_{scale_tag}_{control_mode}_summary.csv"
...
def _apply_initial_qvelrate_perturbation(model, data, perturb_vx, perturb_vy, perturb_yawrate):
    # MuJoCo free-joint qvel convention is used here as a simulation-only perturbation anchor.
    # qvel[0], qvel[1] are used as initial translational velocity perturbations.
    # qvel[5] is used as initial yaw-rate perturbation when available.
    if len(data.qvel) >= 1:
