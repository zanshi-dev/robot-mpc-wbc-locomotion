...
INPUT_CSV = "results/logs_sample/stage07_wbc_posture_regularized_qp.csv"
LOG_CSV = "results/logs_sample/stage07_wbc_posture_regularized_support_test_log.csv"
SUMMARY_CSV = "results/logs_sample/stage07_wbc_posture_regularized_support_test_summary.csv"
MARGIN_CSV = "results/logs_sample/stage07_wbc_posture_regularized_support_test_margin_check.csv"
...
def main():
    ...
    summary = {
        ...
        "pass": str(pass_test),
        "torque_source": "stage07_wbc_posture_regularized_qp",
    }
    ...
    max_roll_row = max(rows, key=lambda r: abs(float(r["roll"])))
    max_pitch_row = max(rows, key=lambda r: abs(float(r["pitch"])))
    min_z_row = min(rows, key=lambda r: float(r["base_z"]))
    last = rows[-1]
    last_100 = rows[-100:]

    margin = {
        "max_abs_roll": f"{abs(float(max_roll_row['roll'])):.12f}",
        "max_abs_roll_step": max_roll_row["step"],
        "roll_margin_to_0p15": f"{0.15 - abs(float(max_roll_row['roll'])):.12f}",
        "max_abs_pitch": f"{abs(float(max_pitch_row['pitch'])):.12f}",
        "max_abs_pitch_step": max_pitch_row["step"],
        "pitch_margin_to_0p15": f"{0.15 - abs(float(max_pitch_row['pitch'])):.12f}",
        "min_z": f"{float(min_z_row['base_z']):.12f}",
        "min_z_step": min_z_row["step"],
        "z_margin_to_0p22": f"{float(min_z_row['base_z']) - 0.22:.12f}",
        "final_z": last["base_z"],
        "final_roll": last["roll"],
        "final_pitch": last["pitch"],
        "roll_abs_last_100_min": f"{min(abs(float(r['roll'])) for r in last_100):.12f}",
        "roll_abs_last_100_max": f"{max(abs(float(r['roll'])) for r in last_100):.12f}",
        "base_z_last_100_min": f"{min(float(r['base_z']) for r in last_100):.12f}",
        "base_z_last_100_max": f"{max(float(r['base_z']) for r in last_100):.12f}",
    }

    margin["pass_margin"] = str(
        float(margin["roll_margin_to_0p15"]) > 0.005
        and float(margin["pitch_margin_to_0p15"]) > 0.005
        and float(margin["z_margin_to_0p22"]) > 0.02
    )

    with open(MARGIN_CSV, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=list(margin.keys()))
        writer.writeheader()
        writer.writerow(margin)

    print("Stage 7 WBC posture regularized support test")
    ...
    print(f"saved_margin={MARGIN_CSV}")
