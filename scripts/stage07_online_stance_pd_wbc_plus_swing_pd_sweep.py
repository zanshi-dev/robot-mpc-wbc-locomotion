...
OUTPUT_CSV = "results/logs_sample/stage07_online_stance_pd_wbc_plus_swing_pd_sweep.csv"
...
def run_case(wbc, target_rows, stance_wbc_scale, swing_target_scale, swing_pd_scale):
    ...
    for step in range(total_steps):
        ...
        max_tau_stance_pd_abs = max(max_tau_stance_pd_abs, float(np.max(np.abs(tau_stance_pd))))
        max_tau_stance_wbc_abs = max(max_tau_stance_wbc_abs, float(np.max(np.abs(tau_stance_wbc))))
        max_tau_swing_pd_abs = max(max_tau_swing_pd_abs, float(np.max(np.abs(tau_swing_pd))))
...
def main():
    ...
    for stance_wbc_scale in STANCE_WBC_SCALE_LIST:
        for swing_pd_scale in SWING_PD_SCALE_LIST:
            for swing_target_scale in SWING_TARGET_SCALE_LIST:
                row = run_case(
                    wbc=wbc,
                ...
                print(
                    "case "
                    f"stance_wbc_scale={stance_wbc_scale} "
                    f"swing_pd_scale={swing_pd_scale} "
                    f"swing_target_scale={swing_target_scale} "
                    f"max_swing_joint_error={row['max_swing_joint_error']} "
    ...
    stance_wbc_pass_cases = sum(
        1 for r in rows
        if r["pass"] == "True" and float(r["stance_wbc_scale"]) > 0.0
    )

    print("Stage 7 online stance PD WBC plus swing PD sweep")
    print(f"saved={OUTPUT_CSV}")
    print(f"num_cases={len(rows)}")
    print(f"pass_cases={pass_cases}")
    print(f"pass_margin_cases={pass_margin_cases}")
    print(f"stance_wbc_pass_cases={stance_wbc_pass_cases}")
    print(
        "recommended "
        f"stance_wbc_scale={recommended['stance_wbc_scale']} "
        f"swing_pd_scale={recommended['swing_pd_scale']} "
        f"swing_target_scale={recommended['swing_target_scale']} "
        f"max_swing_joint_error={recommended['max_swing_joint_error']} "
