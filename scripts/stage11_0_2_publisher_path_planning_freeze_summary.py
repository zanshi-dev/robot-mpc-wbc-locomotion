...
FINAL_GATE = LOG_DIR / "stage11_torque_publisher_safety_gate_after_stage112.csv"
...
DOC_PATH = ROOT / "docs/STAGE11_0_2_PUBLISHER_PATH_PLANNING_FREEZE_SUMMARY.md"
SUMMARY_PATH = LOG_DIR / "stage11_0_2_publisher_path_planning_freeze_summary.csv"
LOG_PATH = LOG_DIR / "stage11_0_2_publisher_path_planning_freeze_log.csv"
HASH_PATH = LOG_DIR / "stage11_0_2_publisher_path_planning_freeze_hashes.csv"
...
def main():
    ...
    add_check(checks, "stage110_publisher_path_implemented", as_bool(s110.get("publisher_path_implemented", "True")), False, not as_bool(s110.get("publisher_path_implemented", "True")))

    add_check(checks, "stage111_publisher_path_source_guard_passed", as_bool(s111.get("publisher_path_source_guard_passed", "False")), True, as_bool(s111.get("publisher_path_source_guard_passed", "False")))
    add_check(checks, "stage111_publisher_path_implemented", as_bool(s111.get("publisher_path_implemented", "True")), False, not as_bool(s111.get("publisher_path_implemented", "True")))

    add_check(checks, "stage112_disabled_publisher_path_design_exists", as_bool(s112.get("disabled_publisher_path_design_exists", "False")), True, as_bool(s112.get("disabled_publisher_path_design_exists", "False")))
    add_check(checks, "stage112_design_all_items_not_implemented", as_bool(s112.get("design_all_items_not_implemented", "False")), True, as_bool(s112.get("design_all_items_not_implemented", "False")))
    add_check(checks, "stage112_publisher_path_implemented", as_bool(s112.get("publisher_path_implemented", "True")), False, not as_bool(s112.get("publisher_path_implemented", "True")))
    ...
    add_check(checks, "stage112_safety_gate_exists", FINAL_GATE.exists(), True, FINAL_GATE.exists(), str(FINAL_GATE))
    for gate, expected in expected_gate_status.items():
        value = gate_status.get(gate, None)
        add_check(checks, f"{gate}_status_after_stage112", value, expected, value == expected)

    torque_enable_ready = all(gate_status.get(gate, False) for gate in expected_gate_status)
    add_check(checks, "torque_enable_ready_after_stage112", torque_enable_ready, False, not torque_enable_ready)
    ...
    DOC_PATH.write_text(f"""# Stage 11.0–11.2 Publisher-path Planning Freeze Summary
...
- G12 publisher path skeleton plan exists: {gate_status.get("G12")}
- G13 publisher-path source guard passed before implementation: {gate_status.get("G13")}
- G14 disabled publisher-path skeleton design exists: {gate_status.get("G14")}
...
""")
    ...
    with SUMMARY_PATH.open("w", newline="") as f:
        ...
        writer.writerow(["stage", "Stage 11.3"])
        writer.writerow(["test_name", "stage11_0_2_publisher_path_planning_freeze_summary"])
        writer.writerow(["stage110_pass", s110.get("pass", "False")])
        writer.writerow(["stage111_pass", s111.get("pass", "False")])
        writer.writerow(["stage112_pass", s112.get("pass", "False")])
        ...
        writer.writerow(["design_all_items_not_implemented", as_bool(s112.get("design_all_items_not_implemented", "False"))])
        ...
        writer.writerow(["g12_publisher_path_plan_exists", gate_status.get("G12", False)])
        writer.writerow(["g13_publisher_path_source_guard_passed", gate_status.get("G13", False)])
        writer.writerow(["g14_disabled_publisher_path_skeleton_design_exists", gate_status.get("G14", False)])
        ...
        writer.writerow(["torque_command_published_by_stage113", False])
        writer.writerow(["stage11_scope", "publisher_path_planning_freeze_summary_only"])
        writer.writerow(["baseline_type", "mixed_online_control_baseline"])
        writer.writerow(["publisher_path_planning_frozen", True])
...
""".strip()
    ...
    marker = "## Stage 11.3 Stage 11.0–11.2 Publisher-path Planning Freeze Summary"
    if marker not in old:
        status_path.write_text(old.rstrip() + "\n\n" + block + "\n")

    print("[Stage 11.3] Stage 11.0-11.2 publisher-path planning freeze summary")
    print(f"pass={all_pass}")
    print(f"missing_file_count={len(missing_files)}")
    print("publisher_path_planning_frozen=True")
