...
DOC_PATH = ROOT / "docs/STAGE11_5_7_DORMANT_PUBLISHER_RUNTIME_FREEZE_SUMMARY.md"
SUMMARY_PATH = LOG_DIR / "stage11_5_7_dormant_publisher_runtime_freeze_summary.csv"
LOG_PATH = LOG_DIR / "stage11_5_7_dormant_publisher_runtime_freeze_log.csv"
HASH_PATH = LOG_DIR / "stage11_5_7_dormant_publisher_runtime_freeze_hashes.csv"
...
def main():
    ...
    add_check(checks, "stage116_dormant_skeleton_exists", as_bool(s116.get("dormant_publisher_path_source_skeleton_exists", "False")), True, as_bool(s116.get("dormant_publisher_path_source_skeleton_exists", "False")))
    ...
    add_check(checks, "stage117_runtime_sample_count", s117.get("runtime_observed_sample_count", ""), "6", s117.get("runtime_observed_sample_count", "") == "6")
    add_check(checks, "stage117_torque_publishers_zero_all_samples", as_bool(s117.get("torque_publishers_zero_all_samples", "False")), True, as_bool(s117.get("torque_publishers_zero_all_samples", "False")))
    add_check(checks, "stage117_topic_info_all_returncode_zero", as_bool(s117.get("topic_info_all_returncode_zero", "False")), True, as_bool(s117.get("topic_info_all_returncode_zero", "False")))
    add_check(checks, "stage117_publisher_path_implemented", as_bool(s117.get("publisher_path_implemented", "True")), False, not as_bool(s117.get("publisher_path_implemented", "True")))
    add_check(checks, "stage117_manual_enable_active", as_bool(s117.get("manual_enable_active", "True")), False, not as_bool(s117.get("manual_enable_active", "True")))
    ...
    add_check(checks, "runtime_observation_csv_exists", (LOG_DIR / "stage11_runtime_guard_hardening_topic_observations.csv").exists(), True, (LOG_DIR / "stage11_runtime_guard_hardening_topic_observations.csv").exists())
    ...
    add_check(checks, "stage117_safety_gate_exists", FINAL_GATE.exists(), True, FINAL_GATE.exists(), str(FINAL_GATE))
    for gate, expected in expected_gate_status.items():
        value = gate_status.get(gate, None)
        add_check(checks, f"{gate}_status_after_stage117", value, expected, value == expected)

    torque_enable_ready = all(gate_status.get(gate, False) for gate in expected_gate_status)
    add_check(checks, "torque_enable_ready_after_stage117", torque_enable_ready, False, not torque_enable_ready)
    ...
    DOC_PATH.write_text(f"""# Stage 11.5–11.7 Dormant Publisher Skeleton Runtime Freeze Summary
...
""")
    ...
    with SUMMARY_PATH.open("w", newline="") as f:
        ...
        writer.writerow(["stage", "Stage 11.8"])
        writer.writerow(["test_name", "stage11_5_7_dormant_publisher_runtime_freeze_summary"])
        writer.writerow(["stage115_pass", s115.get("pass", "False")])
        writer.writerow(["stage116_pass", s116.get("pass", "False")])
        writer.writerow(["stage117_pass", s117.get("pass", "False")])
        ...
        writer.writerow(["torque_command_published_by_stage118", False])
        writer.writerow(["stage11_scope", "dormant_publisher_skeleton_runtime_freeze_summary_only"])
        writer.writerow(["baseline_type", "mixed_online_control_baseline"])
        writer.writerow(["dormant_publisher_skeleton_runtime_frozen", True])
...
- dormant_publisher_runtime_guard_hardened: `{as_bool(s117.get("dormant_publisher_runtime_guard_hardened", "False"))}`
...
""".strip()
    ...
    marker = "## Stage 11.8 Stage 11.5–11.7 Dormant Publisher Skeleton Runtime Freeze Summary"
    if marker not in old:
        status_path.write_text(old.rstrip() + "\n\n" + block + "\n")

    print("[Stage 11.8] Stage 11.5-11.7 dormant publisher skeleton runtime freeze summary")
    print(f"pass={all_pass}")
    print(f"missing_file_count={len(missing_files)}")
    print("dormant_publisher_skeleton_runtime_frozen=True")
    print(f"dormant_publisher_runtime_guard_hardened={as_bool(s117.get('dormant_publisher_runtime_guard_hardened', 'False'))}")
