...
LOG_CSV = "results/logs_sample/stage07_contact_schedule_wbc_scaled_support_test_log.csv"
SUMMARY_CSV = "results/logs_sample/stage07_contact_schedule_wbc_scaled_support_test_summary.csv"
...
def run_mode(mode, tau_wbc):
    ...
    for step in range(SIM_STEPS):
        ...
        rows.append({
            "mode": mode,
        "wbc_scale": MODE_SCALES.get(mode, 1.0),
            "step": step,
            "time": f"{data.time:.9f}",
            "base_z": f"{base_z:.12f}",
            "roll": f"{roll:.12f}",
...
def main():
    ...
    # scale sweep 结果：
    # trot_FR_RL 原始 scale=1.0 失败，max_abs_roll=0.156525005128 > 0.15
    # 推荐 scale=0.6，pass=True 且 pass_margin=True
    for mode, scale in MODE_SCALES.items():
        tau_by_mode[mode] = scale * tau_by_mode[mode]
    ...
    print("Stage 7 contact schedule WBC scaled support test")
    all_pass = True
