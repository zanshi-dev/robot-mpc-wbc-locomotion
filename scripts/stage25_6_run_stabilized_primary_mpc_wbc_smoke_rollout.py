...
def main() -> int:
    ...
    s25_5_summary_path = logs / "stage25_5_stabilized_primary_mpc_wbc_mode_summary.json"
    s25_5 = load_json(s25_5_summary_path)

    execution_csv = logs / "stage25_6_stabilized_primary_mpc_wbc_smoke_execution.csv"
    execution_summary_json = logs / "stage25_6_stabilized_primary_mpc_wbc_smoke_execution_summary.json"
    ...
    trace_csv = logs / "stage25_6_stabilized_primary_mpc_wbc_smoke_trace.csv"
    ...
    cmd = [
        ...
        "--stabilized-primary-scale", "0.05",
        "--stabilized-primary-ramp-steps", "600",
        "--stabilized-posture-residual-scale", "1.0",
        "--stabilized-wbc-residual-scale", "1.0",
        "--scale-tag", scale_tag,
        "--run-id", run_id,
    ...
    expected_summary = logs / f"stage25_5_stabilized_primary_mpc_wbc_rollout_{perturbation_id}_{scale_tag}_{control_mode}_summary.csv"
    expected_log = logs / f"stage25_5_stabilized_primary_mpc_wbc_rollout_{perturbation_id}_{scale_tag}_{control_mode}_log.csv"

    summary_candidates = list(logs.glob(f"stage25_5_stabilized_primary_mpc_wbc_rollout_*_{scale_tag}_{control_mode}_summary.csv"))
    log_candidates = list(logs.glob(f"stage25_5_stabilized_primary_mpc_wbc_rollout_*_{scale_tag}_{control_mode}_log.csv"))
    ...
    stabilized_present = str(runner_summary.get("stabilized_primary_mpc_wbc_mode_present", "")).lower() == "true"
    stabilized_executed = str(runner_summary.get("stabilized_primary_mpc_wbc_executed", "")).lower() == "true"
    ...
    rollout_evidence_generated = summary_exists and log_exists and stabilized_executed
    ...
    execution_summary = {
        "stage": "25.6",
        "name": "stabilized_primary_mpc_wbc smoke rollout execution",
        ...
        "stabilized_primary_mpc_wbc_mode_present": stabilized_present,
        "stabilized_primary_mpc_wbc_executed": stabilized_executed,
        ...
        "runner_log_csv": runner_log_csv_str,
        "trace_csv": str(trace_csv.relative_to(root)) if trace_csv.is_file() else "",
        "stage25_5_result": s25_5.get("result"),
        "stage25_5_stabilized_primary_mpc_wbc_mode_implemented": s25_5.get("stabilized_primary_mpc_wbc_mode_implemented"),
        "claim_boundary": [
            "smoke rollout only",
            "simulation-only stabilized_primary_mpc_wbc runner",
    ...
    print(f"stage25_6_execution_result: {execution_result}")
    ...
    print(f"stabilized_primary_mpc_wbc_executed: {stabilized_executed}")
