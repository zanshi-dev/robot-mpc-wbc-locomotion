...
VIDEO = VIDEOS / "stage13_5a_r1_mujoco_offscreen_2400step_mixed_baseline_demo_720p.mp4"
SUMMARY = OUT / "stage13_5a_r1_mujoco_offscreen_rollout_video_640fb_summary.json"
DOC = DOCS / "stage13_5a_r1_mujoco_offscreen_rollout_video_640fb.md"

DEMO_LOG = OUT / "stage13_5a_r1_video_rollout_mixed_baseline_log.csv"
DEMO_SUMMARY_CSV = OUT / "stage13_5a_r1_video_rollout_mixed_baseline_summary.csv"
...
class Recorder:
    ...
    def start(self, model, mujoco):
        ...
        ffmpeg_cmd = [
            ...
            "-r", str(self.args.fps),
            "-i", "-",
            "-vf", f"scale={self.args.output_width}:-2,pad={self.args.output_width}:{self.args.output_height}:(ow-iw)/2:(oh-ih)/2,format=yuv420p",
            "-an",
            "-c:v", "libx264",
            "-preset", "veryfast",
            "-crf", "18",
...
def run_child(args):
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location("stage13_5a_r1_runner", RUNNER)
    ...
    result = {
        "stage": "13.5A-R1",
        "name": "mujoco_offscreen_rollout_video_640fb",
        ...
        "method": "MuJoCo offscreen rendering at 640x480 + policy rollout + raw RGB pipe to ffmpeg + 720p encode",
    ...
    DOC.write_text(
        "\n".join([
            "# Stage 13.5A-R1 MuJoCo Offscreen Rollout Video",
            "",
            f"- pass: `{result['pass']}`",
...
def run_parent(args):
    ...
    for backend in backends:
        log_path = OUT / f"stage13_5a_r1_backend_{backend}_attempt.log"
        env = os.environ.copy()
        env["MUJOCO_GL"] = backend
        env["STAGE13_5A_R1_CHILD"] = "1"
...
def main():
    ...
    if args.child or os.environ.get("STAGE13_5A_R1_CHILD") == "1":
