...
STAGE116_SUMMARY = LOG_DIR / "stage11_dormant_publisher_path_source_skeleton_freeze_summary.csv"
STAGE115_GATE = LOG_DIR / "stage11_torque_publisher_safety_gate_after_stage115.csv"
...
SUMMARY_PATH = LOG_DIR / "stage11_runtime_guard_hardening_for_dormant_publisher_skeleton_summary.csv"
LOG_PATH = LOG_DIR / "stage11_runtime_guard_hardening_for_dormant_publisher_skeleton_log.csv"
OBS_PATH = LOG_DIR / "stage11_runtime_guard_hardening_topic_observations.csv"
SAFETY_GATE_PATH = LOG_DIR / "stage11_torque_publisher_safety_gate_after_stage117.csv"
DOC_PATH = ROOT / "docs/STAGE11_RUNTIME_GUARD_HARDENING_FOR_DORMANT_PUBLISHER_SKELETON.md"

BUILD_STDOUT = LOG_DIR / "stage11_runtime_guard_hardening_build_stdout.txt"
BUILD_STDERR = LOG_DIR / "stage11_runtime_guard_hardening_build_stderr.txt"
BRIDGE_STDOUT = LOG_DIR / "stage11_runtime_guard_hardening_bridge_stdout.txt"
BRIDGE_STDERR = LOG_DIR / "stage11_runtime_guard_hardening_bridge_stderr.txt"
CONTROLLER_STDOUT = LOG_DIR / "stage11_runtime_guard_hardening_controller_stdout.txt"
CONTROLLER_STDERR = LOG_DIR / "stage11_runtime_guard_hardening_controller_stderr.txt"
PARAM_STDOUT = LOG_DIR / "stage11_runtime_guard_hardening_param_stdout.txt"
PARAM_STDERR = LOG_DIR / "stage11_runtime_guard_hardening_param_stderr.txt"
...
def main():
    ...
    s116 = load_summary(STAGE116_SUMMARY)
    stage116_pass = as_bool(s116.get("pass", "False"))
    dormant_frozen = as_bool(s116.get("dormant_publisher_path_source_skeleton_frozen", "False"))
    dormant_exists_prev = as_bool(s116.get("dormant_publisher_path_source_skeleton_exists", "False"))
    stage116_torque_ready = as_bool(s116.get("torque_enable_ready", "True"))
    stage116_torque_enabled = as_bool(s116.get("torque_publisher_enabled", "True"))
    stage116_control_changed = as_bool(s116.get("control_law_changed", "True"))

    add_check(checks, "stage116_summary_exists", STAGE116_SUMMARY.exists(), True, STAGE116_SUMMARY.exists(), str(STAGE116_SUMMARY))
    add_check(checks, "stage116_pass", stage116_pass, True, stage116_pass)
    add_check(checks, "stage116_dormant_skeleton_frozen", dormant_frozen, True, dormant_frozen)
    add_check(checks, "stage116_dormant_skeleton_exists", dormant_exists_prev, True, dormant_exists_prev)
    add_check(checks, "stage116_torque_enable_ready", stage116_torque_ready, False, not stage116_torque_ready)
    add_check(checks, "stage116_torque_publisher_enabled", stage116_torque_enabled, False, not stage116_torque_enabled)
    add_check(checks, "stage116_control_law_changed", stage116_control_changed, False, not stage116_control_changed)
    ...
    previous_gate_rows = load_gate(STAGE115_GATE)
    previous_gate_status = {
        row.get("gate", ""): as_bool(row.get("current_status", "False"))
        for row in previous_gate_rows
    }

    add_check(checks, "stage115_gate_exists", STAGE115_GATE.exists(), True, STAGE115_GATE.exists(), str(STAGE115_GATE))
    add_check(checks, "stage115_g8_manual_enable_active_false", previous_gate_status.get("G8", True), False, previous_gate_status.get("G8", True) is False)
    add_check(checks, "stage115_g9_active_ros_publisher_path_exists_false", previous_gate_status.get("G9", True), False, previous_gate_status.get("G9", True) is False)
    add_check(checks, "stage115_g16_dormant_skeleton_exists_true", previous_gate_status.get("G16", False), True, previous_gate_status.get("G16", False) is True)
    ...
    dormant_runtime_guard_hardened = (
    ...
    gate_rows = [
        ...
        },
        {
            ...
            "evidence": "parameters remain default false in Stage 11.7",
        },
        {
            "gate": "G9",
            ...
            "evidence": "runtime guard confirms no active publisher in Stage 11.7",
        },
        {
            "gate": "G10",
        ...
        },
        {
            ...
            "required_before_torque_publish": True,
            "current_status": dormant_runtime_guard_hardened,
    ...
    add_check(checks, "dormant_publisher_runtime_guard_hardened", dormant_runtime_guard_hardened, True, dormant_runtime_guard_hardened)
    ...
    DOC_PATH.write_text(f"""# Stage 11.7 Runtime Guard Hardening for Dormant Publisher Skeleton
...
- pass: {stage116_pass}
- dormant_publisher_path_source_skeleton_frozen: {dormant_frozen}
- dormant_publisher_path_source_skeleton_exists: {dormant_exists_prev}
- torque_enable_ready: {stage116_torque_ready}
...
- expected sample count: {runtime_sample_count}
- observed sample count: {observed_sample_count}
...
- G17 runtime guard hardened for dormant publisher skeleton: {dormant_runtime_guard_hardened}
...
""")
    ...
    with SUMMARY_PATH.open("w", newline="") as f:
        ...
        writer.writerow(["stage", "Stage 11.7"])
        writer.writerow(["test_name", "runtime_guard_hardening_for_dormant_publisher_skeleton"])
        writer.writerow(["stage116_pass", stage116_pass])
        writer.writerow(["dormant_publisher_path_source_skeleton_frozen", dormant_frozen])
        ...
        writer.writerow(["dormant_publisher_runtime_guard_hardened", dormant_runtime_guard_hardened])
        ...
        writer.writerow(["g17_runtime_guard_hardened_for_dormant_publisher_skeleton", dormant_runtime_guard_hardened])
        ...
        writer.writerow(["torque_command_published_by_stage117", False])
        writer.writerow(["stage11_scope", "runtime_guard_hardening_for_dormant_publisher_skeleton_only"])
...
- pass: `{all_pass}`
- dormant_publisher_runtime_guard_hardened: `{dormant_runtime_guard_hardened}`
- torque_publishers_zero_all_samples: `{torque_publishers_zero_all_samples}`
...
""".strip()
    ...
    marker = "## Stage 11.7 Runtime Guard Hardening for Dormant Publisher Skeleton"
    if marker not in old:
        status_path.write_text(old.rstrip() + "\n\n" + block + "\n")

    print("[Stage 11.7] runtime guard hardening for dormant publisher skeleton")
    print(f"pass={all_pass}")
    print(f"stage116_pass={stage116_pass}")
    print(f"dormant_publisher_runtime_guard_hardened={dormant_runtime_guard_hardened}")
