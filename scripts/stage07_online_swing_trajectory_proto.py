...
OUTPUT_CSV = "results/logs_sample/stage07_online_swing_trajectory_proto.csv"
SUMMARY_CSV = "results/logs_sample/stage07_online_swing_trajectory_proto_summary.csv"
...
def swing_height(progress):
    return CLEARANCE_Z * np.sin(np.pi * progress)


def swing_target(nominal_pos, progress):
    # 保守原地 swing：x 从 -stride/2 到 +stride/2，z 半正弦抬脚。
    x_offset = STRIDE_X * (progress - 0.5)
    z_offset = swing_height(progress)
...
def main():
    ...
    site_ids, nominal = set_standing_pose(model, data)
    ...
    for step in range(TOTAL_STEPS):
        ...
        for leg in LEG_ORDER:
            if leg in swing_legs:
                target = swing_target(nominal[leg], progress)
                swing_samples[leg] += 1
            else:
                target = nominal[leg].copy()
    ...
    pass_test = z_pass and smooth_pass and balance_pass and transition_pass
    ...
    print("Stage 7 online swing trajectory proto")
