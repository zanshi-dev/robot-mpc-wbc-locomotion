...
## Stage 25：stabilized primary MPC-WBC 主控闭环原型

当前状态：已完成。

Stage 25 的目标是将 MPC/WBC 从 candidate injection 推进到 primary-controller closed-loop prototype，并记录直接主控失败与稳定化主控成功之间的差异。

本阶段结论不是完整工程级 MPC-WBC locomotion controller，而是固定 MuJoCo 仿真设置下的 stabilized primary MPC-WBC 主控闭环原型。

### 阶段结果

| 子阶段 | 内容 | 状态 |
|---|---|---|
| Stage 25.0 | MPC-WBC primary controller roadmap validation | pass |
| Stage 25.1 | control source audit | pass |
| Stage 25.2 | primary_mpc_wbc mode implementation | pass |
| Stage 25.3 | primary_mpc_wbc smoke rollout | expected failure recorded |
| Stage 25.4 | primary_mpc_wbc failure diagnosis | pass |
| Stage 25.5 | stabilized_primary_mpc_wbc mode implementation | pass |
| Stage 25.6 | stabilized_primary_mpc_wbc smoke rollout | pass |
| Stage 25.7 | primary controller closure evidence freeze | pass |

### 关键结论

primary_mpc_wbc 直接主控已经进入 MuJoCo 力矩闭环，但未通过稳定性边界。失败主要来自姿态边界和力矩饱和问题，而不是 QP 求解失败。

stabilized_primary_mpc_wbc 在直接主控基础上加入 ramp transition、scale limit、stance posture residual、online WBC residual、swing leg PD 和 torque safety filter，在固定 MuJoCo 仿真设置下通过 smoke rollout。
...
- 完整工程级 MPC-WBC 四足机器人控制器
- 真实机器人部署
- 硬件 torque enablement
- 复杂地形鲁棒行走
- 外力扰动鲁棒行走
- 无 residual 辅助的纯 MPC/WBC torque 主控稳定行走

### 主要证据文件

- docs/STAGE25_MPC_WBC_PRIMARY_CONTROLLER_ROADMAP.md
- docs/STAGE25_1_CONTROL_SOURCE_AUDIT.md
